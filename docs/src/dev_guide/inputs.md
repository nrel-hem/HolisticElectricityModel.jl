...
Inputs used to run the holistic electrivcity model. It consists of [indexes](#indexes), [parameters](#parameters), [costs](#costs), [earnings](#earnings) and [energy](#energy). [Indexes](#indexes) refer to the dimensions using which the rest of the data is organized, such as the number of generation and storage resources, independent power producers, the balancing areas condsidered, the representative days considered etc.

## Indexes

 - **index_y.csv**: Representative simulation years. It consisits of a single row of year values. The file is used to generate two types of indexes, `index_y` which is the years for a simulation window and `index_y_fix` which represents the full simulation horizon (does not change).
...
## Parameters
 ...
 - **DC_length.csv**: Demand curve length for each year in index\_y. Column names are years (for e.g., `2021, 2022`).
 ...
 - **DER_ITCNewmy.csv**: Multi-year ITC for new DERs. Columns are index\-y, followed byBTM technology names in index\_m (for e.g., `index_y, BTMPV, BTMStorage`).
 ...
 - **ExistingStorDuration.csv**: Duration in hours of existing storage technologies. Column names are the exiting storage technology names (for e.g., `battery_1, battery_2` etc.).
