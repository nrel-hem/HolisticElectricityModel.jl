...
function solve_agent_problem!(
    ...
    for j in model_data.index_j, h in model_data.index_h
        if reg_year == model_data.year(first(model_data.index_y_fix))
            ...
            x_green_cumu[j, h] = sum(
                green_developer.green_tech_buildout_my(Symbol(Int(y_symbol)), j, h) for
                y_symbol = model_data.year(first(model_data.index_y_fix)):(reg_year-1)
            )
        end
    end
    ...
    objective_function = begin
        sum(
            # fixed o&m
            utility.fom_C_my(reg_year_index, j) * x_green[j, h] / (
                green_developer.irr * (1 + green_developer.irr)^20 /
                ((1 + green_developer.irr)^20 - 1)
            ) +
            # capital costs
            utility.CapEx_my(reg_year_index, j) * x_green[j, h] for
            j in model_data.index_j, h in model_data.index_h
        )
    end
    ...
    @constraint(
        ...
        sum(
            ...
            (x_green[j, h] + x_green_cumu[j, h]) for j in model_data.index_j,
            t in model_data.index_t
        ) - customers.x_green_sub_my(reg_year_index, h) / (1 - utility.loss_dist) >= 0
    )

    optimize!(Green_Developer_model)

    green_tech_buildout_before =
        ParamArray(green_developer.green_tech_buildout_my, "green_tech_buildout_before")

    for j in model_data.index_j, h in model_data.index_h
        green_developer.green_tech_buildout_my(reg_year_index, j, h, :) .=
            value.(x_green[j, h])
    end

    for h in model_data.index_h
        if sum(
            green_developer.green_tech_buildout_my(reg_year_index, j, h) for
            j in model_data.index_j
        ) > 0.0
            green_developer.ppa_my(reg_year_index, h, :) .=
                (
                    sum(
                        sum(
                            utility.fom_C_my(reg_year_index, j) *
                            green_developer.green_tech_buildout_my(reg_year_index, j, h) for
                            j in model_data.index_j
                        ) / (1 + green_developer.irr)^n for n = 1:20
                    ) + sum(
                        utility.CapEx_my(reg_year_index, j) *
                        green_developer.green_tech_buildout_my(reg_year_index, j, h) *
                        (1 - utility.ITC_new_my(reg_year_index, j)) for
                        j in model_data.index_j
                    )
                ) / (sum(
...
function welfare_calculation!(
    ...
    green_developer_Revenue = KeyedArray(
        [
            sum(
                green_developer.ppa_my(Symbol(Int(y_symbol)), h) *
                customers.x_green_sub_incremental_my(Symbol(Int(y_symbol)), h) for
                y_symbol =
    ...
    energy_cost = KeyedArray(
        [0.0 for y in model_data.index_y_fix];
        [get_pair(model_data.index_y_fix)]...,
    )
    fixed_om = KeyedArray(
        [
            sum(
                utility.fom_C_my(Symbol(Int(y_symbol)), j) *
                green_developer.green_tech_buildout_my(Symbol(Int(y_symbol)), j, h) for
                y_symbol =
                    model_data.year(first(model_data.index_y_fix)):model_data.year(y),
                h in model_data.index_h, j in model_data.index_j
            ) for y in model_data.index_y_fix
        ];
        [get_pair(model_data.index_y_fix)]...,
    )
    ...
    working_capital = KeyedArray(
        [utility.DaysofWC / 365 * operational_cost(y) for y in model_data.index_y_fix];
        [get_pair(model_data.index_y_fix)]...,
    )
    ...
    rate_base = KeyedArray(
        [
            sum(RateBaseNoWC_new[y, j] for j in model_data.index_j) + working_capital(y) for y in model_data.index_y_fix
        ];
        [get_pair(model_data.index_y_fix)]...,
    )
    debt_interest = KeyedArray(
        [rate_base(y) * utility.DebtRatio * utility.COD for y in model_data.index_y_fix];
        [get_pair(model_data.index_y_fix)]...,
    )
    ...
    depreciation = KeyedArray(
        [
            sum(
                utility.CapEx_my(Symbol(Int(y_symbol)), j) *
                sum(
                    green_developer.green_tech_buildout_my(Symbol(Int(y_symbol)), j, h)
                    for h in model_data.index_h
                ) *
    ...
    green_developer_Cost = KeyedArray(
        [
            debt_interest(y) + income_tax(y) + operational_cost(y) + depreciation(y) for
