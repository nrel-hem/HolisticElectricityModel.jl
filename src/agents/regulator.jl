...
function get_file_prefix(options::RegulatorOptions)
    return join(
        ["$(typeof(options.rate_design))", "$(typeof(options.net_metering_policy))"],
        "_",
    )
end
...
function Regulator(input_filename::String, model_data::HEMData; id = DEFAULT_ID)
    ...
    rep_day_time_tou_mapping =
        CSV.read(joinpath(input_filename, "rep_day_time_tou_mapping.csv"), DataFrame)
...
function solve_agent_problem!(
    ...
    for y in model_data.index_y_fix
        for z in model_data.index_z
            regulator.othercost(z, y, :) .=
                regulator.distribution_cost(z, y) +
                regulator.administration_cost(z, y) +
                regulator.transmission_cost(z, y) +
                regulator.interconnection_cost(z, y) +
                regulator.system_cost(z, y)
        end
    end
    ...
    reg_retirement = sum(
        ...
        y_symbol = model_data.year(first(model_data.index_y_fix)):reg_year
    )
    ...
    for z in model_data.index_z
        energy_cost(z, :) .=
            sum(
                ...
                delta_t *
                (
                    utility.v_E_my(reg_year_index, k, z, d, t) *
                    utility.y_E_my(reg_year_index, k, z, d, t)
                ) for k in utility.index_k_existing, d in model_data.index_d,
                t in model_data.index_t
            ) + sum(
                model_data.omega(d) *
                delta_t *
                (
                    utility.v_C_my(reg_year_index, k, z, d, t) *
                    utility.y_C_my(reg_year_index, k, z, d, t)
                ) for k in utility.index_k_new, d in model_data.index_d,
                t in model_data.index_t
            )
    end
    ...
    for z in model_data.index_z
        fixed_om(z, :) .=
            sum(
                utility.fom_E_my(reg_year_index, z, k) *
                (utility.x_E_my(z, k) - reg_retirement(k, z)) for
            ...
            sum(
                utility.fom_stor_E_my(reg_year_index, z, s) *
                (utility.x_stor_E_my(z, s) - reg_retirement_stor(s, z)) for
    ...
    eximport_cost = make_keyed_array(
        model_data.index_z,
        utility.index_l,
        model_data.index_d,
        model_data.index_t,
    ...
    for z in model_data.index_z,
        ...
        if utility.trans_topology(l, z) * utility.flow_my(reg_year_index, l, d, t) >= 0.0 # export from zone z through line l, l is defined from zone z to somewhere else
            l_sink = utility.trans_topology.dims[2].elements[findall(
                x -> x == -1,
                utility.trans_topology(l, :),
            )[1]]    # get the sink node of line l
            eximport_cost(z, l, d, t, :) .=
                utility.p_energy_cem_my(reg_year_index, l_sink, d, t) *
                utility.trans_topology(l, z) *
                utility.flow_my(reg_year_index, l, d, t) # if price is positive, this should be a positive value
        else    # import into zone z through l
            eximport_cost(z, l, d, t, :) .=
                utility.p_energy_cem_my(reg_year_index, z, d, t) *
                utility.trans_topology(l, z) *
                utility.flow_my(reg_year_index, l, d, t) # if price is positive, this should be a negative value
        end
    end
    ...
    for z in model_data.index_z
        ...
        # export and import cost of modeled areas
            sum(
                model_data.omega(d) * delta_t * eximport_cost(z, l, d, t) for
            ...
            sum(
                ...
                delta_t *
                utility.p_energy_cem_my(reg_year_index, z, d, t) *
                utility.eximport_my(reg_year_index, z, d, t) for d in model_data.index_d,
                t in model_data.index_t
            )
    end
    ...
    for z in model_data.index_z, l in utility.index_l
        ...
        if utility.trans_topology(l, z) * utility.flow_cap_my(reg_year_index, l) >= 0.0 # export from zone z through line l, l is defined from zone z to somewhere else
            ...
            eximport_cap_cost(z, l, :) .=
                utility.p_cap_cem_my(reg_year_index, l_sink) *
                utility.trans_topology(l, z) *
                utility.flow_cap_my(reg_year_index, l) # if price is positive, this should be a positive value
        else    # import into zone z through l
            eximport_cap_cost(z, l, :) .=
                utility.p_cap_cem_my(reg_year_index, z) *
                utility.trans_topology(l, z) *
                utility.flow_cap_my(reg_year_index, l) # if price is positive, this should be a negative value
        end
    end
    ...
    for z in model_data.index_z
        ...
        # export and import cost of modeled areas
            sum(eximport_cap_cost(z, l) for l in utility.index_l) +
            # export and import cost outside of modeled areas
            utility.p_cap_cem_my(reg_year_index, z) * utility.eximport_my(
                reg_year_index,
                z,
                utility.Max_Net_Load_my_dict[reg_year_index, z][1],
                utility.Max_Net_Load_my_dict[reg_year_index, z][2],
            )
    end
    ...
    for z in model_data.index_z
        for k in utility.index_k_new
            ADITNew(z, k, :) .= sum(
                ...
                utility.x_C_my(Symbol(Int(y)), k, z) *
                (1 - utility.CumuITCAmort_new_my(Symbol(Int(reg_year - y + 1)), k)) for
                y = model_data.year(first(model_data.index_y_fix)):reg_year
            )
        end
    end
    ...
    for z in model_data.index_z
        for s in utility.index_stor_new
            ADITStorNew(z, s, :) .= sum(
                ...
                utility.CapEx_stor_my(Symbol(Int(y)), z, s) *
                utility.x_stor_C_my(Symbol(Int(y)), s, z) *
                (1 - utility.CumuITCAmortStor_new_my(Symbol(Int(reg_year - y + 1)), s))
                for y = model_data.year(first(model_data.index_y_fix)):reg_year
            )
        end
    end
    ...
    for z in model_data.index_z
        for s in utility.index_stor_new
            RateBaseNoWCStor_new(z, s, :) .=
                sum(
                    utility.CapEx_stor_my(Symbol(Int(y)), z, s) *
                    utility.x_stor_C_my(Symbol(Int(y)), s, z) *
                    (
                        1 - utility.CumuAccoutDepreStor_new_my(
    ...
    for z in model_data.index_z
        income_tax(z, :) .=
            (
                ...
                (depreciation(z) - depreciation_tax(z)) * utility.Tax - sum(
                    utility.ITC_existing_my(k) *
                    utility.CapEx_existing_my(z, k) *
                    (utility.x_E_my(z, k) - reg_retirement(k, z)) *
                    utility.AnnualITCAmort_existing_my(reg_year_index, k) +
    ...
    for z in model_data.index_z
        ...
        #      50 MW for export. It makes sense to allocate the same cost for internal load and export.
            sum(
                ...
                model_data.omega(d) *
                delta_t *
                customers.d(h, z, d, t) for h in model_data.index_h,
            ...
            sum(
                ...
                delta_t *
                utility.trans_topology(l, z) *
                utility.flow_my(reg_year_index, l, d, t) for l in utility.index_l,
                d in model_data.index_d, t in model_data.index_t
            ) -
            # DG
            # when it comes to cost allocation, simply use total DER generation to offset total load;
            ...
            sum(
                ...
                sum(
                    ...
                    for y_symbol = model_data.year(first(model_data.index_y_fix)):reg_year
    ...
    for z in model_data.index_z, h in model_data.index_h
        # Demand
        net_demand_h_w_loss(z, h, :) .=
            sum(
                customers.gamma(z, h) *
                model_data.omega(d) *
                delta_t *
                customers.d(h, z, d, t) for d in model_data.index_d, t in model_data.index_t
            ) -
            # DG
            sum(
                model_data.omega(d) *
                ...
                ) for m in customers.index_m, d in model_data.index_d,
                t in model_data.index_t
            ) -
            # green technology subscription
            sum(
                ...
                sum(
                    green_developer.green_tech_buildout_my(Symbol(Int(y_symbol)), j, z, h)
                    for y_symbol = model_data.year(first(model_data.index_y_fix)):reg_year
                ) for j in model_data.index_j, d in model_data.index_d,
                t in model_data.index_t
            )
    end
    ...
    for z in model_data.index_z, h in model_data.index_h
        ...
        # Demand without loss
            sum(
                ...
                delta_t *
                customers.d(h, z, d, t) *
                (1 - utility.loss_dist) for d in model_data.index_d, t in model_data.index_t
            ) -
            # DG
            # since this net demand is for rate calculation, we need to consider energy offset at the household level.
            # e.g. two household, with 100 MW load each (without loss), if one of them has DER and generated 
    ...
    net_demand_wo_green_tech_h_wo_loss =
        make_keyed_array(model_data.index_z, model_data.index_h)
    for z in model_data.index_z, h in model_data.index_h
    ...
    for z in model_data.index_z, tou in regulator.index_rate_tou
        ...
        for i =
            1:size(
                regulator.rep_day_time_tou_mapping[
                    (regulator.rep_day_time_tou_mapping.index_rate_tou.==String(tou)),
                    :,
                ],
            )[1]
            d = Symbol(
        ...
        for i =
            ...
            net_eximport_cost_t_temp =
                ...
                sum(
                    ...
                    delta_t *
                    utility.p_energy_cem_my(reg_year_index, z, d, t) *
                    utility.eximport_my(reg_year_index, z, d, t),
                )
        end
    ...
    net_demand_t_w_loss_no_eximport =
        make_keyed_array(model_data.index_z, regulator.index_rate_tou)
    for z in model_data.index_z, tou in regulator.index_rate_tou
        ...
        for i =
            ...
            net_demand_t_w_loss_no_eximport_temp =
                net_demand_t_w_loss_no_eximport_temp +
                # demand
                # when it comes to sharing the revenue requirement (cost), use load including distribution loss
                # e.g. utility generation is 100 MW, 50 MW to serve load (including distribution loss),
    ...
    edo_eximport_demand_t =
        make_keyed_array(model_data.index_z, utility.index_l, regulator.index_rate_tou)
        ...
        for i =
            ...
            edo_eximport_demand_t_temp =
                ...
                sum(
                    ...
                    delta_t *
                    utility.trans_topology(l, z) *
                    utility.flow_my(reg_year_index, l, d, t),
                )
        end
    ...
    der_excess_cost_h_t =
        make_keyed_array(model_data.index_z, model_data.index_h, regulator.index_rate_tou)
    ...
    for z in model_data.index_z, h in model_data.index_h, tou in regulator.index_rate_tou
        ...
        for i =
            ...
            net_demand_h_t_w_loss_temp =
                net_demand_h_t_w_loss_temp + sum(
                    customers.gamma(z, h) *
                    model_data.omega(d) *
                    delta_t *
                    customers.d(h, z, d, t),
    ...
    net_demand_for_peak_h_t = make_keyed_array(
        model_data.index_z,
        model_data.index_h,
        model_data.index_d,
        model_data.index_t,
    ...
    for z in model_data.index_z,
        ...
        net_demand_for_peak_h_t(z, h, d, t, :) .=
            sum(customers.gamma(z, h) * customers.d(h, z, d, t)) -
            # DG
            sum(
                (
                    customers.rho_DG(h, m, z, d, t) *
    ...
    # excluding green tech generation offset when it comes to sharing T&D costs
    net_demand_for_peak_wo_green_tech_h_t = make_keyed_array(
        model_data.index_z,
        model_data.index_h,
        model_data.index_d,
        model_data.index_t,
    ...
    for z in model_data.index_z, h in model_data.index_h
        net_peak_load_h(z, h, :) .= findmax(
            Dict(
                (d, t) => net_demand_for_peak_h_t(z, h, d, t) for
                d in model_data.index_d, t in model_data.index_t
            ),
        )[1]
    end
    ...
    for z in model_data.index_z, h in model_data.index_h
        net_peak_load_wo_green_tech_h(z, h, :) .= findmax(
            Dict(
                (d, t) => net_demand_for_peak_wo_green_tech_h_t(z, h, d, t) for
                d in model_data.index_d, t in model_data.index_t
            ),
        )[1]
    end
    ...
    for z in model_data.index_z, l in utility.index_l
        peak_load_edo_eximport(z, l, :) .= findmax(
            Dict(
                (d, t) =>
                    utility.trans_topology(l, z) * utility.flow_my(reg_year_index, l, d, t) for
                d in model_data.index_d, t in model_data.index_t
            ),
        )[1]
    end
    ...
    for z in model_data.index_z, h in model_data.index_h
        energy_cost_allocation_h(z, h, :) .=
            (energy_cost(z) - net_eximport_cost(z)) * net_demand_h_w_loss(z, h) /
            net_demand_w_loss_no_eximport(z) + der_excess_cost_h(z, h)
    end
    ...
    for z in model_data.index_z, h in model_data.index_h
        demand_cost_allocation_h(z, h, :) .=
            (revenue_requirement(z) - energy_cost(z) + net_eximport_cost(z)) *
            net_peak_load_h(z, h) /
    ...
    for z in model_data.index_z, h in model_data.index_h
        demand_cost_allocation_capacity_h(z, h, :) .=
            (revenue_requirement(z) - energy_cost(z) + net_eximport_cost(z)) *
            net_peak_load_h(z, h) / (sum(net_peak_load_h(z, h) for h in model_data.index_h))
    end
    ...
    for z in model_data.index_z, h in model_data.index_h, tou in regulator.index_rate_tou
        energy_cost_allocation_h_t(z, h, tou, :) .=
            (energy_cost_t(z, tou) - net_eximport_cost_t(z, tou)) *
            net_demand_h_t_w_loss(z, h, tou) / net_demand_t_w_loss_no_eximport(z, tou) +
    ...
    elseif regulator_opts.rate_design isa TOU
        ...
        for z in model_data.index_z,
            ...
            tou = Symbol(
                regulator.rep_day_time_tou_mapping[
                    (regulator.rep_day_time_tou_mapping.index_d.==String(
                        d,
...
function solve_agent_problem!(
    ...
    for z in model_data.index_z
        ...
        #      50 MW for export. It makes sense to allocate the same cost for internal load and export.
            ...
            sum(
                model_data.omega(d) *
                delta_t *
                ipp.trans_topology(l, z) *
                ipp.flow_my(reg_year_index, l, d, t) for l in utility.index_l,
                d in model_data.index_d, t in model_data.index_t
            ) -
            # DG
            # when it comes to cost allocation, simply use total DER generation to offset total load;
        ...
        for i =
            ...
            net_demand_t_w_loss_temp =
                ...
                sum(
                    model_data.omega(d) *
                    delta_t *
                    ipp.eximport_my(reg_year_index, z, d, t),
                ) +
                # endogenous export/import (flow out of zone z)
                sum(
                    model_data.omega(d) *
        ...
        for i =
            ...
            edo_eximport_demand_t_temp =
                ...
                sum(
                    ...
                    delta_t *
                    ipp.trans_topology(l, z) *
                    ipp.flow_my(reg_year_index, l, d, t),
                )
        end
        edo_eximport_demand_t(z, l, tou, :) .= edo_eximport_demand_t_temp
    end
    ...
    for z in model_data.index_z, h in model_data.index_h, tou in regulator.index_rate_tou
        ...
        net_demand_wo_green_tech_h_t_wo_loss(z, h, tou, :) .=
            net_demand_wo_green_tech_h_t_wo_loss_temp
    end
    ...
    local_net_load = make_keyed_array(
        model_data.index_z,
        model_data.index_h,
        model_data.index_d,
        model_data.index_t,
    ...
    for z in model_data.index_z,
        ...
        local_net_load(z, h, d, t, :) .=
            customers.gamma(z, h) * customers.d(h, z, d, t) - sum(
                customers.rho_DG(h, m, z, d, t) *
                customers.x_DG_E_my(first(model_data.index_y), h, z, m) + sum(
                    customers.rho_DG(h, m, z, d, t) *
                    customers.x_DG_new_my(Symbol(Int(y)), h, z, m) for
    ...
    energy_purchase_quantity_detailed =
        make_keyed_array(model_data.index_z, model_data.index_d, model_data.index_t)
    for z in model_data.index_z, d in model_data.index_d, t in model_data.index_t
        ...
        # aggregate across consumer class, then take the maximum of net load and zero.
            max(0.0, sum(local_net_load(z, h, d, t) for h in model_data.index_h)) +
            # exogenous export
            max(0.0, ipp.eximport_my(reg_year_index, z, d, t)) +
            # endogenous export 
            sum(
                max(0.0, ipp.trans_topology(l, z) * ipp.flow_my(reg_year_index, l, d, t))
                for l in utility.index_l
            ) +
            # storage charge
            sum(
                ipp.charge_E_my(reg_year_index, p, s, z, d, t) for
                s in ipp.index_stor_existing, p in ipp.index_p
            ) +
            sum(
                ipp.charge_C_my(reg_year_index, p, s, z, d, t) for s in ipp.index_stor_new,
                p in ipp.index_p
            )
    end
    ...
    energy_purchase_quantity_detailed_tou =
        make_keyed_array(model_data.index_z, regulator.index_rate_tou)
    for z in model_data.index_z, tou in regulator.index_rate_tou
        ...
        for i =
            ...
            energy_purchase_quantity_detailed_tou_temp =
                energy_purchase_quantity_detailed_tou_temp +
                # positive net load
                model_data.omega(d) *
                delta_t *
                    ...
                    sum(
                        (
                            customers.gamma(z, h) * customers.d(h, z, d, t) - sum(
                                ...
                                sum(
                                    customers.rho_DG(h, m, z, d, t) *
                                    customers.x_DG_new_my(Symbol(Int(y)), h, z, m) for y =
                                        model_data.year(first(model_data.index_y_fix)):reg_year
                                ) for m in customers.index_m
                            )
                        ) for h in model_data.index_h
                ...
                sum(
                    max(
                        0.0,
                        ipp.trans_topology(l, z) * ipp.flow_my(reg_year_index, l, d, t),
                    ) for l in utility.index_l
                ) +
                # storage charge
                model_data.omega(d) *
                ...
                delta_t *
                sum(
                    ipp.charge_C_my(reg_year_index, p, s, z, d, t) for
                    s in ipp.index_stor_new, p in ipp.index_p
                )
        end
        energy_purchase_quantity_detailed_tou(z, tou, :) .=
            energy_purchase_quantity_detailed_tou_temp
    end

    energy_purchase_cost = make_keyed_array(model_data.index_z, model_data.index_h)
    for z in model_data.index_z, h in model_data.index_h
        energy_purchase_cost(z, h, :) .= sum(
            model_data.omega(d) *
            delta_t *
            ipp.LMP_my(reg_year_index, z, d, t) *
            local_net_load(z, h, d, t) for d in model_data.index_d,
            t in model_data.index_t
        )
    end
    ...
    for z in model_data.index_z
        ...
        # incorporate REC cost into energy purchase cost
            sum(
                ...
                (
                    sum(
                        ipp.y_E_my(reg_year_index, p, rps, z, d, t) for
                        rps in ipp.index_rps, p in ipp.index_p
                    ) + sum(
                        ipp.y_C_my(reg_year_index, p, rps, z, d, t) for
                        rps in ipp.index_rps, p in ipp.index_p
                    )
                ) for d in model_data.index_d, t in model_data.index_t
            )
    ...
    capacity_purchase_cost =
        ipp.capacity_price(reg_year_index) * ipp.ucap_total(reg_year_index)
    ...
    for z in model_data.index_z, h in model_data.index_h
        energy_cost_allocation_h(z, h, :) .=
            ...
            rec_purchase_cost(z) * sum(
                model_data.omega(d) * delta_t * local_net_load(z, h, d, t) for
                d in model_data.index_d, t in model_data.index_t
            ) / sum(
                model_data.omega(d) * delta_t * energy_purchase_quantity_detailed(z, d, t)
                for d in model_data.index_d, t in model_data.index_t
            ) +
    ...
    for z in model_data.index_z, h in model_data.index_h
        demand_cost_allocation_h(z, h, :) .=
            capacity_purchase_cost * net_peak_load_wo_green_tech_h(z, h) / sum(
                net_peak_load_wo_green_tech_h(z, h) for z in model_data.index_z,
                h in model_data.index_h
            ) +
            regulator.othercost(z, reg_year_index) * net_peak_load_wo_green_tech_h(z, h) /
            (sum(net_peak_load_wo_green_tech_h(z, h) for h in model_data.index_h))
    ...
    for z in model_data.index_z, h in model_data.index_h, tou in regulator.index_rate_tou
        energy_cost_allocation_h_t(z, h, tou, :) .=
            ...
            rec_purchase_cost_t(z, tou) * local_net_load_tou(z, h, tou) /
            energy_purchase_quantity_detailed_tou(z, tou) +
            der_excess_cost_h_t(z, h, tou)
    end
    replace!(energy_cost_allocation_h_t, NaN => 0.0)
    ...
    for z in model_data.index_z, h in model_data.index_h
        p_before_wavg(z, h, :) .=
            sum(
                regulator.p_my(reg_year_index, z, h, d, t) *
                model_data.omega(d) *
                delta_t *
                customers.d(h, z, d, t) for d in model_data.index_d, t in model_data.index_t
            ) / sum(
                model_data.omega(d) * delta_t * customers.d(h, z, d, t) for
                d in model_data.index_d, t in model_data.index_t
            )
    end
