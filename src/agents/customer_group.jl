...
function CustomerGroup(input_filename::AbstractString, model_data::HEMData; id = DEFAULT_ID)
    ...
    demand = read_param("d", input_filename, "Demand", model_data.index_t, [model_data.index_h])
    demand_my = read_param("d_my", input_filename, "Demandmy", model_data.index_t, [model_data.index_y, model_data.index_h])
    x_DG_E = read_param(
        "x_DG_E",
        input_filename,
        "ExistingDER",
    ...
    x_DG_E_my = read_param("x_DG_E_my", input_filename, "ExistingDERmy", index_m, [model_data.index_y, model_data.index_h])
    Opti_DG =
        read_param("Opti_DG", input_filename, "OptimalDER", index_m, [model_data.index_h])
    Opti_DG_my = read_param("Opti_DG_my", input_filename, "OptimalDERmy", index_m, [model_data.index_y, model_data.index_h])
    rho_DG = read_param(
        "rho_DG",
        input_filename,
        "AvailabilityDER",
    ...
    MaxLoad = Dict(h => gamma[h] * findmax(Dict(t => demand[h,t] for t in model_data.index_t))[1]
    for h in model_data.index_h)
    MaxLoad_my = Dict((y,h) => gamma[h] * findmax(Dict(t => demand_my[y,h,t] for t in model_data.index_t))[1]
    ...
    # Customer financing
    debt_ratio = read_param("debt_ratio", input_filename, "CustomerDebtRatio", model_data.index_h)
    cost_of_debt = read_param("cost_of_debt", input_filename, "CustomerCOD", model_data.index_h)
    cost_of_equity = read_param("cost_of_equity", input_filename, "CustomerCOE", model_data.index_h)
    tax_rate = read_param("tax_rate", input_filename, "CustomerTax", model_data.index_h)
    atwacc = Dict(h => debt_ratio[h]*cost_of_debt[h]*(1-tax_rate[h])+(1-debt_ratio[h])*cost_of_equity[h] for h in model_data.index_h)
    CRF = Dict(h => atwacc[h]*(1+atwacc[h])^20/((1+atwacc[h])^20-1) for h in model_data.index_h)
    ...
    return CustomerGroup(
        ...
        read_param("CapEx_DG_my", input_filename, "CapExDERmy", index_m, [model_data.index_y, model_data.index_h]),
        read_param("FOM_DG", input_filename, "FOMDER", index_m, [model_data.index_h]),
        read_param("FOM_DG_my", input_filename, "FOMDERmy", index_m, [model_data.index_y, model_data.index_h]),
        rho_DG,
        ParamScalar("delta", 0.05),
        ParamVector("PeakLoad", model_data.index_h, MaxLoad),
        ParamArray("PeakLoad_my", Tuple(push!(copy([model_data.index_y]), model_data.index_h)), MaxLoad_my),
        ...
        initialize_param("ConPVNetSurplus_my", model_data.index_y, model_data.index_h, index_m),
        initialize_param("GreenTechIntercept", model_data.index_h, model_data.index_j, value=3.5), # Intercept of green tech demand curve
        initialize_param("GreenTechSlope", model_data.index_h, model_data.index_j, value=-0.07),
        pv_adoption_model,
...
function solve_agent_problem!(
    ...
    empty!(x_DG_before)
    merge!(x_DG_before, Dict((h,m) => customers.x_DG_new_my[reg_year_index,h,m] for h in model_data.index_h, m in customers.index_m))
    ...
    for h in model_data.index_h, m in customers.index_m
        ...
        if w_iter >= 2
            ...
            sum(customers.x_DG_new_my[Symbol(Int(y)),h,m] for y = model_data.year[first(model_data.index_y_fix)]:reg_year-1)
        else
            customers.x_DG_E[h,m] = customers.x_DG_E_my[reg_year_index,h,m]
        end
    end
    ...
    NetProfit = Dict((h,m) =>
        ...
        sum(model_data.omega[t] * regulator.p[h,t] * min(customers.d[h,t]*(1-utility.loss_dist), customers.rho_DG[h,m,t] * customers.Opti_DG[h,m]) for t in model_data.index_t) +
        # value of distributed generation (excess generation)
        sum(model_data.omega[t] * regulator.p_ex[h,t] * max(0, customers.rho_DG[h,m,t] * customers.Opti_DG[h,m] - customers.d[h,t]*(1-utility.loss_dist)) for t in model_data.index_t) -
...
function welfare_calculation!(
    ...
    # The NetProfit represents the energy saving/credit per representative agent per DER technology, assuming the optimal DER technology size
    NetProfit = Dict((y,h,m) =>
        # value of distributed generation (offset load)
        sum(model_data.omega[t] * regulator.p_my[y,h,t] * min(customers.d_my[y,h,t]*(1-utility.loss_dist), customers.rho_DG[h,m,t] * customers.Opti_DG_my[y,h,m]) 
            for t in model_data.index_t) +
        # value of distributed generation (excess generation)
        sum(model_data.omega[t] * regulator.p_ex_my[y,h,t] * max(0, customers.rho_DG[h,m,t] * customers.Opti_DG_my[y,h,m] - customers.d_my[y,h,t]*(1-utility.loss_dist))
            for t in model_data.index_t) -
        # cost of distributed generation 
        customers.FOM_DG_my[y,h,m] * customers.Opti_DG_my[y,h,m] for y in model_data.index_y_fix, h in model_data.index_h, m in customers.index_m)
    ...
    for y in model_data.index_y_fix, h in model_data.index_h, m in customers.index_m
        if NetProfit[y,h,m] >= 0.0
            # Calculate total Net Consumer Surplus of PV installation
            Integral = Dict((y,h,m) =>
                QuadGK.quadgk(x -> customers.gamma[h]*customers.Opti_DG_my[y,h,m] * (1-Distributions.cdf(Distributions.Gamma(adopt_model.Shape[h,m], 1/adopt_model.Rate[h,m]*NetProfit[y,h,m]/customers.Opti_DG_my[y,h,m]),x)), 
                customers.CapEx_DG_my[y,h,m], 100*customers.CapEx_DG_my[y,h,m], rtol=1e-8))
            # Calculate annualized Net Consumer Surplus of PV installation
            if customers.MaxDG_my[y,h,m] == 0.0
                customers.ConPVNetSurplus_my[y,h,m] = 0.0
            else
                customers.ConPVNetSurplus_my[y,h,m] =
                    customers.delta * customers.x_DG_new_my[y,h,m]/customers.MaxDG_my[y,h,m]*Integral[y,h,m][1]
            end
        else
            customers.ConPVNetSurplus_my[y,h,m] = 0.0
        end
    ...
    EnergySaving = Dict((y,h,m) =>
        sum(
            sum(model_data.omega[t] * regulator.p_my[y,h,t] * min(customers.d_my[y,h,t]*(1-utility.loss_dist), customers.rho_DG[h,m,t] * customers.Opti_DG_my[Symbol(Int(y_star)),h,m]) 
                for t in model_data.index_t) * customers.x_DG_new_my[Symbol(Int(y_star)),h,m] / customers.Opti_DG_my[Symbol(Int(y_star)),h,m] for y_star = model_data.year[first(model_data.index_y_fix)]:model_data.year[y]
        )
        for y in model_data.index_y_fix, h in model_data.index_h, m in customers.index_m
    )
    ...
    EnergyCost = Dict((y,h,m) =>
        sum(model_data.omega[t]*regulator.p_my[y,h,t]*(customers.gamma[h]*customers.d_my[y,h,t]*(1-utility.loss_dist) - 
        # savings from new DERs
        sum(
            min(customers.d_my[y,h,t]*(1-utility.loss_dist), customers.rho_DG[h,m,t] * customers.Opti_DG_my[Symbol(Int(y_star)),h,m]) * 
            customers.x_DG_new_my[Symbol(Int(y_star)),h,m] / customers.Opti_DG_my[Symbol(Int(y_star)),h,m] for y_star = model_data.year[first(model_data.index_y_fix)]:model_data.year[y]
        )
    ...
    EnergyCost_eximport = Dict(y =>
        sum(model_data.omega[t]*regulator.p_eximport_my[y,t]*utility.eximport_my[y,t] for t in model_data.index_t)
        for y in model_data.index_y_fix)
    
    # Finally, calculate Net Consumer Surplus
    ConNetSurplus = Dict((y,h,m) =>
        sum(customers.ConPVNetSurplus_my[Symbol(Int(y_star)),h,m] for y_star = model_data.year[first(model_data.index_y_fix)]:model_data.year[y]) - 
        EnergySaving[y,h,m] - EnergyCost[y,h,m] for y in model_data.index_y_fix, h in model_data.index_h, m in customers.index_m)
    # Sum of Net Consumer Surplus across customer tpye and DER technology
    TotalConNetSurplus =  Dict(y => 
        sum(ConNetSurplus[y,h,m] for h in model_data.index_h, m in customers.index_m) - EnergyCost_eximport[y]
        for y in model_data.index_y_fix)

    ConPVNetSurplus_PerCustomer_my = Dict((y,h,m) => 
        customers.ConPVNetSurplus_my[y,h,m] / (customers.x_DG_new_my[y,h,m]/customers.Opti_DG_my[y,h,m])
        for y in model_data.index_y_fix, h in model_data.index_h, m in customers.index_m)
    AnnualBill_PerCustomer_my = Dict((y,h) => 
        sum(model_data.omega[t]*regulator.p_my[y,h,t]*customers.d_my[y,h,t]*(1-utility.loss_dist) for t in model_data.index_t)
        for y in model_data.index_y_fix, h in model_data.index_h)
    AverageBill_PerCustomer_my = Dict((y,h) =>
        AnnualBill_PerCustomer_my[y,h] / sum(model_data.omega[t]*customers.d_my[y,h,t]*(1-utility.loss_dist) for t in model_data.index_t)
        for y in model_data.index_y_fix, h in model_data.index_h) 

    return customers.ConPVNetSurplus_my, EnergySaving, EnergyCost, ConNetSurplus, TotalConNetSurplus, ConPVNetSurplus_PerCustomer_my, AnnualBill_PerCustomer_my, AverageBill_PerCustomer_my
