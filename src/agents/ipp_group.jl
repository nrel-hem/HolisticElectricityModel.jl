...
function IPPGroup(input_filename::String, model_data::HEMData, id = DEFAULT_ID)
    ...
    return IPPGroup(
        ...
        ParamArray(
            "f_C",
            Tuple(push!(copy([index_p, model_data.index_z]), index_k_new)),
            FixedCostNew,
        ),
        read_param(
            "v_E",
...
function solve_agent_problem_ipp_cap(
    ...
    WMDER_IPP = get_new_jump_model(ipp_opts.solvers["solve_agent_problem_ipp_mppdc"])
    MPPDCMER_lower =
        get_new_jump_model(ipp_opts.solvers["solve_agent_problem_ipp_mppdc_mccormic_lower"])
    MPPDCMER_lower_dual =
        get_new_jump_model(ipp_opts.solvers["solve_agent_problem_ipp_mppdc_mccormic_lower"])
    ...
    objective_function_lower = begin
        sum(
            sum(
                ...
                z in model_data.index_z, k in ipp.index_k_existing, p in ipp.index_p
            ) + sum(
                model_data.omega(d) *
                delta_t *
                (ipp.v_C_my(y, p, k, z, d, t) * y_C_bounds[y, p, k, z, d, t]) for
                d in model_data.index_d, t in model_data.index_t,
                z in model_data.index_z, k in ipp.index_k_new, p in ipp.index_p
            ) for y in model_data.index_y
        )
    end
    ...
    supply_demand_balance_lower =
        (y, z, d, t) -> begin
            # bulk generation at time t
            sum(y_E_bounds[y, p, k, z, d, t] for k in ipp.index_k_existing, p in ipp.index_p) + sum(
                y_C_bounds[y, p, k, z, d, t] for k in ipp.index_k_new, p in ipp.index_p
            ) -
            # flow out of zone z
            sum(ipp.trans_topology(l, z) * flow_bounds[y, l, d, t] for l in ipp.index_l) +
            # battery discharge
            sum(
                discharge_E_bounds[y, p, s, z, d, t] for s in ipp.index_stor_existing,
                p in ipp.index_p
            ) +
            sum(
                discharge_C_bounds[y, p, s, z, d, t] for s in ipp.index_stor_new,
                p in ipp.index_p
            ) -
            # battery charge
            sum(
                charge_E_bounds[y, p, s, z, d, t] for s in ipp.index_stor_existing,
                p in ipp.index_p
            ) - sum(
                charge_C_bounds[y, p, s, z, d, t] for s in ipp.index_stor_new,
                p in ipp.index_p
            ) -
            # demand at time t
            sum(
            ...
            # green technology subscription at time t
            sum(
                ipp.rho_C_my(Symbol("ipp1"), j, z, d, t) * sum(
    ...
    @constraint(
        MPPDCMER_lower,
        Eq_primal_feasible_supplydemandbalance_lower[
            y in model_data.index_y,
    ...
    @constraint(
        ...
        ipp.rho_E_my(p, k, z, d, t) * (
            ipp.x_E_my(p, z, k) - sum(
                ipp.x_R_my(Symbol(Int(y_symbol)), p, k, z) for
    ...
    @constraint(
        ...
        ipp.rho_C_my(p, k, z, d, t) * (
            sum(
                ipp.x_C_my(Symbol(Int(y_symbol)), p, k, z) for
                y_symbol = model_data.year(first(model_data.index_y)):model_data.year(y)
            ) + ipp.x_C_cumu(p, k, z)
    ...
    @constraint(
        ...
        ] - discharge_E_bounds[y, p, s, z, d, t] / ipp.rte_stor_E_my(y, p, z, s) * delta_t +
    ...
    @constraint(
        ...
        ipp.stor_duration_existing(s) * (
            ipp.x_stor_E_my(p, z, s) - sum(
                ipp.x_stor_R_my(Symbol(Int(y_symbol)), p, s, z) for
                y_symbol = model_data.year(first(model_data.index_y)):model_data.year(y)
    ...
    @constraint(
        ...
        ] >= discharge_E_bounds[y, p, s, z, d, t] * delta_t
    )
    ...
    @constraint(
        ...
        ipp.x_stor_E_my(p, z, s) - sum(
            ipp.x_stor_R_my(Symbol(Int(y_symbol)), p, s, z) for
            y_symbol = model_data.year(first(model_data.index_y)):model_data.year(y)
        ) - ipp.x_stor_R_cumu(p, s, z) - charge_E_bounds[y, p, s, z, d, t] -
        discharge_E_bounds[y, p, s, z, d, t] / ipp.rte_stor_E_my(y, p, z, s) >= 0
    )
    ...
    @constraint(
        ...
        ] - discharge_C_bounds[y, p, s, z, d, t] / ipp.rte_stor_C_my(y, p, z, s) * delta_t +
    ...
    @constraint(
        ...
        ipp.stor_duration_new(s) * (
            sum(
                ipp.x_stor_C_my(Symbol(Int(y_symbol)), p, s, z) for
                y_symbol = model_data.year(first(model_data.index_y)):model_data.year(y)
    ...
    @constraint(
        ...
        ] - discharge_C_bounds[y, p, s, z, d, t] * delta_t >= 0
    )
    ...
    @constraint(
        ...
        sum(
            ipp.x_stor_C_my(Symbol(Int(y_symbol)), p, s, z) for
            y_symbol = model_data.year(first(model_data.index_y)):model_data.year(y)
        ) + ipp.x_stor_C_cumu(p, s, z) - charge_C_bounds[y, p, s, z, d, t] -
        discharge_C_bounds[y, p, s, z, d, t] / ipp.rte_stor_C_my(y, p, z, s) >= 0
    )
        ...
        sum(
            sum(
                miu_lower[y, z, d, t] * (
                    ...
                    # existing DG generation at time t
                    sum(
                        customers.rho_DG(h, m, z, d, t) * customers.x_DG_E_my(y, h, z, m) for h in model_data.index_h,
                        m in customers.index_m
                    ) -
                    # new DG generation at time t
                    sum(
                        customers.rho_DG(h, m, z, d, t) * sum(
                            customers.x_DG_new_my(Symbol(Int(y_symbol)), h, z, m) for
                            y_symbol =
                    ...
                    sum(
                        ipp.rho_C_my(Symbol("ipp1"), j, z, d, t) * sum(
                            green_developer.green_tech_buildout_my(
                                Symbol(Int(y_symbol)),
                                j,
                                z,
                                h,
                ...
                ) for z in model_data.index_z, d in model_data.index_d,
                t in model_data.index_t
            ) - (sum(
                ...
                (
                    ipp.x_E_my(p, z, k) - sum(
                        ipp.x_R_my(Symbol(Int(y_symbol)), p, k, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) - ipp.x_R_cumu(p, k, z)
                ) for p in ipp.index_p, k in ipp.index_k_existing,
                z in model_data.index_z, d in model_data.index_d,
                t in model_data.index_t
            )) - (sum(
                ...
                (
                    sum(
                        ipp.x_C_my(Symbol(Int(y_symbol)), p, k, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) + ipp.x_C_cumu(p, k, z)
                ...
                sum(
                    iota_min_lower[y, l, d, t] * ipp.trans_capacity(l, :min) for
                    l in ipp.index_l, d in model_data.index_d, t in model_data.index_t
                ) - sum(
                    iota_max_lower[y, l, d, t] * ipp.trans_capacity(l, :max) for
                    l in ipp.index_l, d in model_data.index_d, t in model_data.index_t
                )
            ) - (sum(
                ...
                (
                    ipp.x_stor_E_my(p, z, s) - sum(
                        ipp.x_stor_R_my(Symbol(Int(y_symbol)), p, s, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) - ipp.x_stor_R_cumu(p, s, z)
                ) for p in ipp.index_p, s in ipp.index_stor_existing,
                z in model_data.index_z, d in model_data.index_d,
                t in model_data.index_t
            )) - (sum(
                ...
                (
                    sum(
                        ipp.x_stor_C_my(Symbol(Int(y_symbol)), p, s, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) + ipp.x_stor_C_cumu(p, s, z)
                ) for p in ipp.index_p, s in ipp.index_stor_new,
                z in model_data.index_z, d in model_data.index_d,
                t in model_data.index_t
            )) - (sum(
                ...
                psi_E_lower[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_existing, z in model_data.index_z,
                d in model_data.index_d
            ) - sum(
                pi_E_discharge_lower[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_existing, z in model_data.index_z,
                d in model_data.index_d
            ) +
            sum(
                pi_E_charge_lower[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_existing, z in model_data.index_z,
                d in model_data.index_d
            ) +
            sum(
                psi_C_lower[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_new, z in model_data.index_z,
                d in model_data.index_d
            ) - sum(
                pi_C_discharge_lower[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_new, z in model_data.index_z,
                d in model_data.index_d
            ) + sum(
                pi_C_charge_lower[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_new, z in model_data.index_z,
                d in model_data.index_d
            ) for y in model_data.index_y
    ...
    @constraint(
        ...
        model_data.omega(d) * delta_t * ipp.v_E_my(y, p, k, z, d, t) -
        miu_lower[y, z, d, t] + eta_lower[y, p, k, z, d, t] >= 0
    )
    @constraint(
        MPPDCMER_lower_dual,
        ...
        model_data.omega(d) * delta_t * ipp.v_C_my(y, p, k, z, d, t) -
        miu_lower[y, z, d, t] + lambda_lower[y, p, k, z, d, t] >= 0
    )
    @constraint(
        MPPDCMER_lower_dual,
        ...
        sum(miu_lower[y, z, d, t] * ipp.trans_topology(l, z) for z in model_data.index_z) -
        iota_min_lower[y, l, d, t] + iota_max_lower[y, l, d, t] == 0
    )
    @constraint(
        MPPDCMER_lower_dual,
        ...
        -miu_lower[y, z, d, t] -
        psi_E_lower[y, p, s, z, d, t] / ipp.rte_stor_E_my(y, p, z, s) * delta_t +
        theta_E_discharge_lower[y, p, s, z, d, t] +
        pi_E_discharge_lower[y, p, s, z, d, t] * delta_t +
        kappa_E_lower[y, p, s, z, d, t] / ipp.rte_stor_E_my(y, p, z, s) >= 0
    )
    @constraint(
        MPPDCMER_lower_dual,
        ...
        miu_lower[y, z, d, t] +
        psi_E_lower[y, p, s, z, d, t] * delta_t +
        theta_E_charge_lower[y, p, s, z, d, t] +
        pi_E_charge_lower[y, p, s, z, d, t] * delta_t +
        kappa_E_lower[y, p, s, z, d, t] >= 0
    )
    @constraint(
        MPPDCMER_lower_dual,
        ...
        ] - psi_E_lower[y, p, s, z, d, t] + theta_E_energy_lower[y, p, s, z, d, t] -
        pi_E_discharge_lower[
            y,
            p,
            s,
    ...
    @constraint(
        ...
        -miu_lower[y, z, d, t] -
        psi_C_lower[y, p, s, z, d, t] / ipp.rte_stor_C_my(y, p, z, s) * delta_t +
        theta_C_discharge_lower[y, p, s, z, d, t] +
        pi_C_discharge_lower[y, p, s, z, d, t] * delta_t +
        kappa_C_lower[y, p, s, z, d, t] / ipp.rte_stor_C_my(y, p, z, s) >= 0
    )
    @constraint(
        MPPDCMER_lower_dual,
        ...
        miu_lower[y, z, d, t] +
        psi_C_lower[y, p, s, z, d, t] * delta_t +
        theta_C_charge_lower[y, p, s, z, d, t] +
        pi_C_charge_lower[y, p, s, z, d, t] * delta_t +
        kappa_C_lower[y, p, s, z, d, t] >= 0
    )
    @constraint(
        MPPDCMER_lower_dual,
        ...
        ] - psi_C_lower[y, p, s, z, d, t] + theta_C_energy_lower[y, p, s, z, d, t] -
        pi_C_discharge_lower[
            y,
            p,
            s,
    ...
    for y in model_data.index_y,
        ...
        eta_param(y, k, z, d, t, :) .=
            abs(dual.(Eq_primal_feasible_gen_max_E_lower[y, p_star, k, z, d, t]))
    end

    lambda_param = initialize_param(
        "lambda_param",
        model_data.index_y,
        ipp.index_k_new,
        model_data.index_z,
    ...
    for y in model_data.index_y,
        ...
        lambda_param(y, k, z, d, t, :) .=
            abs(dual.(Eq_primal_feasible_gen_max_C_lower[y, p_star, k, z, d, t]))
    end

    theta_E_energy_param = initialize_param(
        "theta_E_energy_param",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
        model_data.index_d,
    ...
    for y in model_data.index_y,
        ...
        theta_E_energy_param(y, s, z, d, t, :) .=
            abs(dual.(Eq_primal_feasible_energy_upper_bound_E_lower[y, p_star, s, z, d, t]))
    end

    theta_E_discharge_param = initialize_param(
        "theta_E_discharge_param",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
        model_data.index_d,
    ...
    for y in model_data.index_y,
        ...
        theta_E_discharge_param(y, s, z, d, t, :) .= abs(
            dual.(Eq_primal_feasible_discharge_upper_bound_E_lower[y, p_star, s, z, d, t]),
        )
    end

    theta_E_charge_param = initialize_param(
        "theta_E_charge_param",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
        model_data.index_d,
    ...
    for y in model_data.index_y,
        ...
        theta_E_charge_param(y, s, z, d, t, :) .=
            abs(dual.(Eq_primal_feasible_charge_upper_bound_E_lower[y, p_star, s, z, d, t]))
    end

    pi_E_charge_param = initialize_param(
        "pi_E_charge_param",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    for y in model_data.index_y,
        ...
        pi_E_charge_param(y, s, z, d, t, :) .= abs(
            dual.(
                Eq_primal_feasible_charge_energy_upper_bound_E_lower[y, p_star, s, z, d, t]
            ),
        )
    end
    for y in model_data.index_y,
    ...
    kappa_E_param = initialize_param(
        "kappa_E_param",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    theta_C_energy_param = initialize_param(
        "theta_C_energy_param",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
        model_data.index_d,
    ...
    for y in model_data.index_y,
        ...
        theta_C_energy_param(y, s, z, d, t, :) .=
            abs(dual.(Eq_primal_feasible_energy_upper_bound_C_lower[y, p_star, s, z, d, t]))
    end

    theta_C_discharge_param = initialize_param(
        "theta_C_discharge_param",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
        model_data.index_d,
    ...
    for y in model_data.index_y,
        ...
        theta_C_discharge_param(y, s, z, d, t, :) .= abs(
            dual.(Eq_primal_feasible_discharge_upper_bound_C_lower[y, p_star, s, z, d, t]),
        )
    end

    theta_C_charge_param = initialize_param(
        "theta_C_charge_param",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
        model_data.index_d,
    ...
    for y in model_data.index_y,
        ...
        theta_C_charge_param(y, s, z, d, t, :) .=
            abs(dual.(Eq_primal_feasible_charge_upper_bound_C_lower[y, p_star, s, z, d, t]))
    end

    pi_C_charge_param = initialize_param(
        "pi_C_charge_param",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    for y in model_data.index_y,
        ...
        pi_C_charge_param(y, s, z, d, t, :) .= abs(
            dual.(
                Eq_primal_feasible_charge_energy_upper_bound_C_lower[y, p_star, s, z, d, t]
            ),
        )
    end
    for y in model_data.index_y,
    ...
    kappa_C_param = initialize_param(
        "kappa_C_param",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
        ...
        for k in ipp.index_k_existing
            for z in model_data.index_z
                for d in model_data.index_d
                    for t in model_data.index_t
                        eta_U(y, k, z, d, t, :) .=
                            eta_param(y, k, z, d, t) * eta_upper_bound_adj
                        eta_L(y, k, z, d, t, :) .=
                            eta_param(y, k, z, d, t) * eta_lower_bound_adj
                    end
                end
            end
        end
    ...
    X_R_cumu_L = initialize_param(
        "x_R_L",
        model_data.index_y,
        ipp.index_k_existing,
        model_data.index_z,
    )
    X_R_cumu_U = initialize_param(
        "x_R_U",
        model_data.index_y,
        ipp.index_k_existing,
        model_data.index_z,
    ...
    fill!(X_R_cumu_U, NaN)
    X_R_stor_cumu_L = initialize_param(
        "x_R_stor_L",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    )
    X_R_stor_cumu_U = initialize_param(
        "x_R_stor_U",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    for y in model_data.index_y
        for k in ipp.index_k_existing
            for z in model_data.index_z
                ...
                if k == Symbol("nuclear")
                    X_R_cumu_U(y, k, z, :) .=
                        min(ipp.x_E_my(p_star, z, k) - ipp.x_R_cumu(p_star, k, z), 1.0)
                else
                    X_R_cumu_U(y, k, z, :) .=
                        ipp.x_E_my(p_star, z, k) - ipp.x_R_cumu(p_star, k, z)
                end
            end
        end
    end
    for y in model_data.index_y
        for s in ipp.index_stor_existing
            for z in model_data.index_z
                # need to have constraints in the optimization as well
                X_R_stor_cumu_U(y, s, z, :) .=
                    ipp.x_stor_E_my(p_star, z, s) - ipp.x_stor_R_cumu(p_star, s, z)
            end
        end
    end

    lambda_L = initialize_param(
        "lambda_L",
        model_data.index_y,
        ipp.index_k_new,
        model_data.index_z,
        ...
        for k in ipp.index_k_new
            for z in model_data.index_z
                for d in model_data.index_d
                    for t in model_data.index_t
                        lambda_U(y, k, z, d, t, :) .=
                            lambda_param(y, k, z, d, t) * lambda_upper_bound_adj
                        lambda_L(y, k, z, d, t, :) .=
                            lambda_param(y, k, z, d, t) * lambda_lower_bound_adj
                    end
                end
            end
        end
    ...
    X_C_cumu_L =
        initialize_param("x_C_L", model_data.index_y, ipp.index_k_new, model_data.index_z)
    X_C_cumu_U =
        initialize_param("x_C_U", model_data.index_y, ipp.index_k_new, model_data.index_z)
    X_C_stor_cumu_L = initialize_param(
        "x_C_stor_L",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    )
    X_C_stor_cumu_U = initialize_param(
        "x_C_stor_U",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    theta_E_energy_L = initialize_param(
        "theta_E_energy_L",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    theta_E_energy_U = initialize_param(
        "theta_E_energy_U",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    theta_E_discharge_L = initialize_param(
        "theta_E_discharge_L",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    theta_E_discharge_U = initialize_param(
        "theta_E_discharge_U",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    theta_E_charge_L = initialize_param(
        "theta_E_charge_L",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    theta_E_charge_U = initialize_param(
        "theta_E_charge_U",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    pi_E_charge_L = initialize_param(
        "pi_E_charge_L",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    pi_E_charge_U = initialize_param(
        "pi_E_charge_U",
        model_data.index_y,
        ipp.index_stor_existing,
        model_data.index_z,
    ...
    for y in model_data.index_y
        for k in ipp.index_stor_existing
            for z in model_data.index_z
                for d in model_data.index_d
                    for t in model_data.index_t
                        theta_E_energy_U(y, k, z, d, t, :) .=
                            theta_E_energy_param(y, k, z, d, t) * battery_upper_bound_adj
                        theta_E_energy_L(y, k, z, d, t, :) .=
                            theta_E_energy_param(y, k, z, d, t) * battery_lower_bound_adj
                        theta_E_discharge_U(y, k, z, d, t, :) .=
                            theta_E_discharge_param(y, k, z, d, t) * battery_upper_bound_adj
                        theta_E_discharge_L(y, k, z, d, t, :) .=
                            theta_E_discharge_param(y, k, z, d, t) * battery_lower_bound_adj
                        theta_E_charge_U(y, k, z, d, t, :) .=
                            theta_E_charge_param(y, k, z, d, t) * battery_upper_bound_adj
                        theta_E_charge_L(y, k, z, d, t, :) .=
                            theta_E_charge_param(y, k, z, d, t) * battery_lower_bound_adj
                        pi_E_charge_U(y, k, z, d, t, :) .=
                            pi_E_charge_param(y, k, z, d, t) * battery_upper_bound_adj
                        pi_E_charge_L(y, k, z, d, t, :) .=
                            pi_E_charge_param(y, k, z, d, t) * battery_lower_bound_adj
                        kappa_E_U(y, k, z, d, t, :) .=
                            kappa_E_param(y, k, z, d, t) * battery_upper_bound_adj
                        kappa_E_L(y, k, z, d, t, :) .=
                            kappa_E_param(y, k, z, d, t) * battery_lower_bound_adj
                    end
                end
            end
        end
    ...
    theta_C_energy_L = initialize_param(
        "theta_C_energy_L",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    theta_C_energy_U = initialize_param(
        "theta_C_energy_U",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    theta_C_discharge_L = initialize_param(
        "theta_C_discharge_L",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    theta_C_discharge_U = initialize_param(
        "theta_C_discharge_U",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    theta_C_charge_L = initialize_param(
        "theta_C_charge_L",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    theta_C_charge_U = initialize_param(
        "theta_C_charge_U",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    pi_C_charge_L = initialize_param(
        "pi_C_charge_L",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    pi_C_charge_U = initialize_param(
        "pi_C_charge_U",
        model_data.index_y,
        ipp.index_stor_new,
        model_data.index_z,
    ...
    for y in model_data.index_y
        for k in ipp.index_stor_new
            for z in model_data.index_z
                for d in model_data.index_d
                    for t in model_data.index_t
                        theta_C_energy_U(y, k, z, d, t, :) .=
                            theta_C_energy_param(y, k, z, d, t) * battery_upper_bound_adj
                        theta_C_energy_L(y, k, z, d, t, :) .=
                            theta_C_energy_param(y, k, z, d, t) * battery_lower_bound_adj
                        theta_C_discharge_U(y, k, z, d, t, :) .=
                            theta_C_discharge_param(y, k, z, d, t) * battery_upper_bound_adj
                        theta_C_discharge_L(y, k, z, d, t, :) .=
                            theta_C_discharge_param(y, k, z, d, t) * battery_lower_bound_adj
                        theta_C_charge_U(y, k, z, d, t, :) .=
                            theta_C_charge_param(y, k, z, d, t) * battery_upper_bound_adj
                        theta_C_charge_L(y, k, z, d, t, :) .=
                            theta_C_charge_param(y, k, z, d, t) * battery_lower_bound_adj
                        pi_C_charge_U(y, k, z, d, t, :) .=
                            pi_C_charge_param(y, k, z, d, t) * battery_upper_bound_adj
                        pi_C_charge_L(y, k, z, d, t, :) .=
                            pi_C_charge_param(y, k, z, d, t) * battery_lower_bound_adj
                        kappa_C_U(y, k, z, d, t, :) .=
                            kappa_C_param(y, k, z, d, t) * battery_upper_bound_adj
                        kappa_C_L(y, k, z, d, t, :) .=
                            kappa_C_param(y, k, z, d, t) * battery_lower_bound_adj
                    end
                end
            end
        end
    ...
    @variable(
        WMDER_IPP,
        x_R[model_data.index_y, ipp.index_k_existing, model_data.index_z] >= 0
    )
    @variable(
        WMDER_IPP,
        x_stor_C[model_data.index_y, ipp.index_stor_new, model_data.index_z] >= 0
    )
    @variable(
        WMDER_IPP,
        x_stor_R[model_data.index_y, ipp.index_stor_existing, model_data.index_z] >= 0
    )
    ...
    @variable(
        WMDER_IPP,
        mce_theta_E_energy_x_stor_R_p_star[
            model_data.index_y,
            ipp.index_stor_existing,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        mce_theta_E_discharge_x_stor_R_p_star[
            model_data.index_y,
            ipp.index_stor_existing,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        mce_theta_E_charge_x_stor_R_p_star[
            model_data.index_y,
            ipp.index_stor_existing,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        mce_pi_E_charge_x_stor_R_p_star[
            model_data.index_y,
            ipp.index_stor_existing,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        mce_theta_C_energy_x_stor_C_p_star[
            model_data.index_y,
            ipp.index_stor_new,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        mce_theta_C_discharge_x_stor_C_p_star[
            model_data.index_y,
            ipp.index_stor_new,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        mce_theta_C_charge_x_stor_C_p_star[
            model_data.index_y,
            ipp.index_stor_new,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        mce_pi_C_charge_x_stor_C_p_star[
            model_data.index_y,
            ipp.index_stor_new,
            model_data.index_z,
            model_data.index_d,
    ...
    @variable(
        WMDER_IPP,
        x_R_mce[model_data.index_y, ipp.index_k_existing, model_data.index_z] >= 0
    )
    @variable(
        WMDER_IPP,
        x_stor_R_mce[model_data.index_y, ipp.index_stor_existing, model_data.index_z] >= 0
    )
    # cumulative investment of p_star
    @variable(
        WMDER_IPP,
        0 <= x_C_mce[model_data.index_y, ipp.index_k_new, model_data.index_z] <= 5000.0
    )
    @variable(
        WMDER_IPP,
        0 <=
        x_stor_C_mce[model_data.index_y, ipp.index_stor_new, model_data.index_z] <=
        5000.0
    )
    ...
    @variable(
        WMDER_IPP,
        iota_min[model_data.index_y, ipp.index_l, model_data.index_d, model_data.index_t] >=
        0
    )
    @variable(
        WMDER_IPP,
        iota_max[model_data.index_y, ipp.index_l, model_data.index_d, model_data.index_t] >=
        0
    )
    @variable(
        WMDER_IPP,
    ...
    @variable(WMDER_IPP, flow_cap[model_data.index_y, ipp.index_l])
    ...
    for y in model_data.index_y, z in model_data.index_z
        ipp.Max_Net_Load_my(y, z, :) .= findmax(
            Dict(
                (d, t) => ipp.Net_Load_my(y, z, d, t) for d in model_data.index_d,
                t in model_data.index_t
            ),
        )[1]
        push!(
    ...
    for y in model_data.index_y
        ipp.Capacity_slope_my(y, :) .=
            -ipp.NetCONE(y) /
            (ipp.DC_length(y) * sum(ipp.Reserve_req_my(y, z) for z in model_data.index_z))
        ipp.Capacity_intercept_my(y, :) .=
            -ipp.Capacity_slope_my(y) *
            sum(ipp.Reserve_req_my(y, z) for z in model_data.index_z) + ipp.NetCONE(y)
    end
    ...
    if length(ipp.index_p) >= 2
        UCAP_total =
            y -> begin
                ...
                # green technology subscription
                sum(
                    ipp.capacity_credit_C_my(y, z, j) * sum(
                        green_developer.green_tech_buildout_my(
                            Symbol(Int(y_symbol)),
                            j,
                            z,
    ...
    objective_function = begin
        sum(
            ...
            sum(
                ...
                x_C[y, k, z] *
                sum(
                    ipp.pvf_onm(Symbol(Int(y_symbol)), p_star) for y_symbol =
            ...
            ipp.pvf_cap(y, p_star) * sum(
                ipp.CapEx_my(y, p_star, z, k) * x_C[y, k, z] for k in ipp.index_k_new,
                z in model_data.index_z
            ) -
            # fixed costs
            ...
            ipp.pvf_cap(y, p_star) * sum(
                ipp.CapEx_stor_my(y, p_star, z, s) * x_stor_C[y, s, z] for
                s in ipp.index_stor_new, z in model_data.index_z
            ) +
            # Linearized profit term
            ipp.pvf_onm(y, p_star) * (
                sum(
                    miu[y, z, d, t] * (
                        sum(
                            ...
                            customers.x_DG_E_my(y, h, z, m) for h in model_data.index_h,
                            m in customers.index_m
                        ) - sum(
                            customers.rho_DG(h, m, z, d, t) * sum(
                                customers.x_DG_new_my(Symbol(Int(y_symbol)), h, z, m)
                                for y_symbol =
                                    model_data.year(first(model_data.index_y_fix)):model_data.year(
                                        y,
                                    )
                    ...
                    sum(
                        ...
                        (
                            ipp.x_E_my(p_star, z, k) - sum(
                                ipp.x_R_my(Symbol(Int(y_symbol)), p_star, k, z) for
                                y_symbol =
                                    model_data.year(first(model_data.index_y)):model_data.year(
                                        y,
                                    )
                    ...
                    sum(
                        ...
                        (
                            sum(
                                ipp.x_C_my(Symbol(Int(y_symbol)), p_star, k, z) for
                                y_symbol =
                                    model_data.year(first(model_data.index_y)):model_data.year(
                                        y,
                                    )
                    ...
                    iota_min[y, l, d, t] * ipp.trans_capacity(l, :min) for
                    l in ipp.index_l, d in model_data.index_d, t in model_data.index_t
                ) - sum(
                    iota_max[y, l, d, t] * ipp.trans_capacity(l, :max) for
                    l in ipp.index_l, d in model_data.index_d, t in model_data.index_t
                ) - (
                    sum(
                        ...
                        (
                            ipp.x_stor_E_my(p_star, z, s) - sum(
                                ipp.x_stor_R_my(Symbol(Int(y_symbol)), p_star, s, z) for
                                y_symbol =
                                    model_data.year(first(model_data.index_y)):model_data.year(
                                        y,
                                    )
                    ...
                    sum(
                        ...
                        (
                            sum(
                                ipp.x_stor_C_my(Symbol(Int(y_symbol)), p_star, s, z) for
                                y_symbol =
                                    model_data.year(first(model_data.index_y)):model_data.year(
                                        y,
                                    )
                ...
                (
                    sum(
                        ...
                        d in model_data.index_d, t in model_data.index_t,
                        z in model_data.index_z, k in ipp.index_k_existing,
                        p in ipp.index_p
                    ) + sum(
                        model_data.omega(d) *
                        delta_t *
    ...
    @constraint(
        ...
        model_data.omega(d) * delta_t * ipp.v_E_my(y, p, k, z, d, t) - miu[y, z, d, t] +
        eta[y, p, k, z, d, t] >= 0
    )
    @constraint(
        WMDER_IPP,
        ...
        model_data.omega(d) * delta_t * ipp.v_C_my(y, p, k, z, d, t) - miu[y, z, d, t] +
        lambda[y, p, k, z, d, t] >= 0
    )
    @constraint(
        WMDER_IPP,
        ...
        sum(miu[y, z, d, t] * ipp.trans_topology(l, z) for z in model_data.index_z) -
        iota_min[y, l, d, t] + iota_max[y, l, d, t] == 0
    )
    @constraint(
        WMDER_IPP,
        ...
        -miu[y, z, d, t] -
        psi_E[y, p, s, z, d, t] / ipp.rte_stor_E_my(y, p, z, s) * delta_t +
        theta_E_discharge[y, p, s, z, d, t] +
        pi_E_discharge[y, p, s, z, d, t] * delta_t +
        kappa_E[y, p, s, z, d, t] / ipp.rte_stor_E_my(y, p, z, s) >= 0
    )
    @constraint(
        WMDER_IPP,
        ...
        miu[y, z, d, t] +
        psi_E[y, p, s, z, d, t] * delta_t +
        theta_E_charge[y, p, s, z, d, t] +
        pi_E_charge[y, p, s, z, d, t] * delta_t +
        kappa_E[y, p, s, z, d, t] >= 0
    )
    @constraint(
        WMDER_IPP,
        ...
        ] - psi_E[y, p, s, z, d, t] + theta_E_energy[y, p, s, z, d, t] - pi_E_discharge[
            y,
            p,
            s,
            z,
    ...
    @constraint(
        ...
        -miu[y, z, d, t] -
        psi_C[y, p, s, z, d, t] / ipp.rte_stor_C_my(y, p, z, s) * delta_t +
        theta_C_discharge[y, p, s, z, d, t] +
        pi_C_discharge[y, p, s, z, d, t] * delta_t +
        kappa_C[y, p, s, z, d, t] / ipp.rte_stor_C_my(y, p, z, s) >= 0
    )
    @constraint(
        WMDER_IPP,
        ...
        miu[y, z, d, t] +
        psi_C[y, p, s, z, d, t] * delta_t +
        theta_C_charge[y, p, s, z, d, t] +
        pi_C_charge[y, p, s, z, d, t] * delta_t +
        kappa_C[y, p, s, z, d, t] >= 0
    )
    @constraint(
        WMDER_IPP,
        ...
        ] - psi_C[y, p, s, z, d, t] + theta_C_energy[y, p, s, z, d, t] - pi_C_discharge[
            y,
            p,
            s,
            z,
    ...
    supply_demand_balance =
        (y, z, d, t) -> begin
            ...
            # battery discharge
            sum(
                discharge_E[y, p, s, z, d, t] for s in ipp.index_stor_existing,
                p in ipp.index_p
            ) +
            sum(discharge_C[y, p, s, z, d, t] for s in ipp.index_stor_new, p in ipp.index_p) -
            # battery charge
            sum(
                charge_E[y, p, s, z, d, t] for s in ipp.index_stor_existing,
                p in ipp.index_p
            ) - sum(
                charge_C[y, p, s, z, d, t] for s in ipp.index_stor_new, p in ipp.index_p
            ) -
            # demand at time t
            sum(
                customers.gamma(z, h) * customers.d_my(y, h, z, d, t) for
    ...
    @constraint(
        WMDER_IPP,
        Eq_primal_feasible_supplydemandbalance[
            y in model_data.index_y,
            z in model_data.index_z,
            d in model_data.index_d,
            t in model_data.index_t,
    ...
    @constraint(
        ...
        ] - discharge_E[y, p, s, z, d, t] / ipp.rte_stor_E_my(y, p, z, s) * delta_t +
    ...
    @constraint(
        ...
        discharge_E[y, p, s, z, d, t] * delta_t <= energy_E[
            y,
            p,
            s,
            z,
    ...
    @constraint(
        ...
        charge_E[y, p_star, s, z, d, t] * delta_t <=
        ipp.stor_duration_existing(s) * (
            ipp.x_stor_E_my(p_star, z, s) - sum(
                x_stor_R[Symbol(Int(y_symbol)), s, z] for
                y_symbol = model_data.year(first(model_data.index_y)):model_data.year(y)
    ...
    @constraint(
        ...
        ] - discharge_C[y, p, s, z, d, t] / ipp.rte_stor_C_my(y, p, z, s) * delta_t +
    ...
    @constraint(
        ...
        discharge_C[y, p, s, z, d, t] * delta_t <= energy_C[
            y,
            p,
            s,
            z,
    ...
    @constraint(
        ...
        discharge_C[y, p, s, z, d, t] * delta_t <= ipp.initial_energy_new_my(y, p, s, z, d)
    )
    ...
    @constraint(
        ...
        charge_C[y, p_star, s, z, d, t] * delta_t <=
        ipp.stor_duration_new(s) * (
            sum(
                x_stor_C[Symbol(Int(y_symbol)), s, z] for
                y_symbol = model_data.year(first(model_data.index_y)):model_data.year(y)
    ...
    if length(ipp.index_p) >= 2
        ...
        @constraint(
            WMDER_IPP,
            Eq_strong_duality[y in model_data.index_y],
            sum(
                model_data.omega(d) *
                delta_t *
                (ipp.v_E_my(y, p, k, z, d, t) * y_E[y, p, k, z, d, t]) for
                d in model_data.index_d, t in model_data.index_t, z in model_data.index_z,
            ...
            sum(
                miu[y, z, d, t] * (
                    ...
                    # existing DG generation at time t
                    sum(
                        customers.rho_DG(h, m, z, d, t) * customers.x_DG_E_my(y, h, z, m)
                        for h in model_data.index_h, m in customers.index_m
                    ) -
                    # new DG generation at time t
                    sum(
                ...
                sum(
                    eta[y, p, k, z, d, t] *
                    ipp.rho_E_my(p, k, z, d, t) *
                    (ipp.x_E_my(p, z, k) - ipp.x_R_cumu(p, k, z)) for p in ipp.index_p,
                    k in ipp.index_k_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                ) - sum(
                    ipp.rho_E_my(p_star, k, z, d, t) * mce_eta_x_R_p_star[y, k, z, d, t] for
                    k in ipp.index_k_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    lambda[y, p, k, z, d, t] *
                    ipp.rho_C_my(p, k, z, d, t) *
                    ipp.x_C_cumu(p, k, z) for p in ipp.index_p, k in ipp.index_k_new,
                    z in model_data.index_z, d in model_data.index_d,
                    t in model_data.index_t
                ) + sum(
                ...
                sum(
                    iota_min[y, l, d, t] * ipp.trans_capacity(l, :min) for l in ipp.index_l,
                    d in model_data.index_d, t in model_data.index_t
                ) - sum(
                    iota_max[y, l, d, t] * ipp.trans_capacity(l, :max) for l in ipp.index_l,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    theta_E_energy[y, p, s, z, d, t] *
                    ipp.stor_duration_existing(s) *
                    (ipp.x_stor_E_my(p, z, s) - ipp.x_stor_R_cumu(p, s, z)) for
                    p in ipp.index_p, s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                ) - sum(
                    ipp.stor_duration_existing(s) *
                    mce_theta_E_energy_x_stor_R_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    ...
                    ipp.rte_stor_E_my(y, p_star, z, s) *
                    mce_theta_E_discharge_x_stor_R_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    ...
                    mce_theta_E_charge_x_stor_R_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    ...
                    ipp.stor_duration_existing(s) *
                    mce_pi_E_charge_x_stor_R_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    ...
                    mce_kappa_E_x_stor_R_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    theta_C_energy[y, p, s, z, d, t] *
                    ipp.stor_duration_new(s) *
                    ipp.x_stor_C_cumu(p, s, z) for p in ipp.index_p,
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                ) + sum(
                    ipp.stor_duration_new(s) *
                    mce_theta_C_energy_x_stor_C_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    ...
                    ipp.rte_stor_C_my(y, p_star, z, s) *
                    mce_theta_C_discharge_x_stor_C_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    theta_C_charge[y, p, s, z, d, t] * ipp.x_stor_C_cumu(p, s, z) for
                    p in ipp.index_p, s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                ) + sum(
                    mce_theta_C_charge_x_stor_C_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    ...
                    ipp.stor_duration_new(s) *
                    mce_pi_C_charge_x_stor_C_p_star[y, s, z, d, t] for
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                )
            ) - (
                sum(
                    kappa_C[y, p, s, z, d, t] * ipp.x_stor_C_cumu(p, s, z) for
                    p in ipp.index_p, s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d, t in model_data.index_t
                ) + sum(
                    mce_kappa_C_x_stor_C_p_star[y, s, z, d, t] for s in ipp.index_stor_new,
                    z in model_data.index_z, d in model_data.index_d,
                    t in model_data.index_t
                )
            ) + sum(
                psi_E[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_existing, z in model_data.index_z,
                d in model_data.index_d
            ) - sum(
                pi_E_discharge[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_existing, z in model_data.index_z,
                d in model_data.index_d
            ) +
            sum(
                pi_E_charge[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_existing, z in model_data.index_z,
                d in model_data.index_d
            ) +
            sum(
                psi_C[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_new, z in model_data.index_z, d in model_data.index_d
            ) - sum(
                pi_C_discharge[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_new, z in model_data.index_z, d in model_data.index_d
            ) + sum(
                pi_C_charge[y, p, s, z, d, model_data.index_t.elements[1]] *
                ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                s in ipp.index_stor_new, z in model_data.index_z, d in model_data.index_d
            )
        )
    ...
    @constraint(
        ...
        mce_eta_x_R_p_star[y, k, z, d, t] >=
        X_R_cumu_L(y, k, z) * eta[y, p_star, k, z, d, t] +
        eta_L(y, k, z, d, t) * x_R_mce[y, k, z] -
        eta_L(y, k, z, d, t) * X_R_cumu_L(y, k, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_eta_x_R_p_star[y, k, z, d, t] >=
        -eta_U(y, k, z, d, t) * X_R_cumu_U(y, k, z) +
        eta_U(y, k, z, d, t) * x_R_mce[y, k, z] +
        eta[y, p_star, k, z, d, t] * X_R_cumu_U(y, k, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_eta_x_R_p_star[y, k, z, d, t] <=
        X_R_cumu_U(y, k, z) * eta[y, p_star, k, z, d, t] -
        eta_L(y, k, z, d, t) * X_R_cumu_U(y, k, z) +
        eta_L(y, k, z, d, t) * x_R_mce[y, k, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_eta_x_R_p_star[y, k, z, d, t] <=
        eta_U(y, k, z, d, t) * x_R_mce[y, k, z] -
        eta_U(y, k, z, d, t) * X_R_cumu_L(y, k, z) +
        X_R_cumu_L(y, k, z) * eta[y, p_star, k, z, d, t]
    )
    ...
    @constraint(
        ...
        mce_lambda_x_C_p_star[y, k, z, d, t] >=
        X_C_cumu_L(y, k, z) * lambda[y, p_star, k, z, d, t] +
        lambda_L(y, k, z, d, t) * x_C_mce[y, k, z] -
        lambda_L(y, k, z, d, t) * X_C_cumu_L(y, k, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_lambda_x_C_p_star[y, k, z, d, t] >=
        -lambda_U(y, k, z, d, t) * X_C_cumu_U(y, k, z) +
        lambda_U(y, k, z, d, t) * x_C_mce[y, k, z] +
        lambda[y, p_star, k, z, d, t] * X_C_cumu_U(y, k, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_lambda_x_C_p_star[y, k, z, d, t] <=
        X_C_cumu_U(y, k, z) * lambda[y, p_star, k, z, d, t] -
        lambda_L(y, k, z, d, t) * X_C_cumu_U(y, k, z) +
        lambda_L(y, k, z, d, t) * x_C_mce[y, k, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_lambda_x_C_p_star[y, k, z, d, t] <=
        lambda_U(y, k, z, d, t) * x_C_mce[y, k, z] -
        lambda_U(y, k, z, d, t) * X_C_cumu_L(y, k, z) +
        X_C_cumu_L(y, k, z) * lambda[y, p_star, k, z, d, t]
    )
    ...
    @constraint(
        ...
        mce_theta_E_energy_x_stor_R_p_star[y, s, z, d, t] >=
        X_R_stor_cumu_L(y, s, z) * theta_E_energy[y, p_star, s, z, d, t] +
        theta_E_energy_L(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        theta_E_energy_L(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_energy_x_stor_R_p_star[y, s, z, d, t] >=
        -theta_E_energy_U(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        theta_E_energy_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] +
        theta_E_energy[y, p_star, s, z, d, t] * X_R_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_energy_x_stor_R_p_star[y, s, z, d, t] <=
        X_R_stor_cumu_U(y, s, z) * theta_E_energy[y, p_star, s, z, d, t] -
        theta_E_energy_L(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        theta_E_energy_L(y, s, z, d, t) * x_stor_R_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_energy_x_stor_R_p_star[y, s, z, d, t] <=
        theta_E_energy_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        theta_E_energy_U(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z) +
        X_R_stor_cumu_L(y, s, z) * theta_E_energy[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        mce_theta_E_discharge_x_stor_R_p_star[y, s, z, d, t] >=
        X_R_stor_cumu_L(y, s, z) * theta_E_discharge[y, p_star, s, z, d, t] +
        theta_E_discharge_L(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        theta_E_discharge_L(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_discharge_x_stor_R_p_star[y, s, z, d, t] >=
        -theta_E_discharge_U(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        theta_E_discharge_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] +
        theta_E_discharge[y, p_star, s, z, d, t] * X_R_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_discharge_x_stor_R_p_star[y, s, z, d, t] <=
        X_R_stor_cumu_U(y, s, z) * theta_E_discharge[y, p_star, s, z, d, t] -
        theta_E_discharge_L(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        theta_E_discharge_L(y, s, z, d, t) * x_stor_R_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_discharge_x_stor_R_p_star[y, s, z, d, t] <=
        theta_E_discharge_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        theta_E_discharge_U(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z) +
        X_R_stor_cumu_L(y, s, z) * theta_E_discharge[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        X_R_stor_cumu_L(y, s, z) * theta_E_charge[y, p_star, s, z, d, t] +
        theta_E_charge_L(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        theta_E_charge_L(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_charge_x_stor_R_p_star[y, s, z, d, t] >=
        -theta_E_charge_U(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        theta_E_charge_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] +
        theta_E_charge[y, p_star, s, z, d, t] * X_R_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_charge_x_stor_R_p_star[y, s, z, d, t] <=
        X_R_stor_cumu_U(y, s, z) * theta_E_charge[y, p_star, s, z, d, t] -
        theta_E_charge_L(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        theta_E_charge_L(y, s, z, d, t) * x_stor_R_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_E_charge_x_stor_R_p_star[y, s, z, d, t] <=
        theta_E_charge_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        theta_E_charge_U(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z) +
        X_R_stor_cumu_L(y, s, z) * theta_E_charge[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        mce_pi_E_charge_x_stor_R_p_star[y, s, z, d, t] >=
        X_R_stor_cumu_L(y, s, z) * pi_E_charge[y, p_star, s, z, d, t] +
        pi_E_charge_L(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        pi_E_charge_L(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_pi_E_charge_x_stor_R_p_star[y, s, z, d, t] >=
        -pi_E_charge_U(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        pi_E_charge_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] +
        pi_E_charge[y, p_star, s, z, d, t] * X_R_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_pi_E_charge_x_stor_R_p_star[y, s, z, d, t] <=
        X_R_stor_cumu_U(y, s, z) * pi_E_charge[y, p_star, s, z, d, t] -
        pi_E_charge_L(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        pi_E_charge_L(y, s, z, d, t) * x_stor_R_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_pi_E_charge_x_stor_R_p_star[y, s, z, d, t] <=
        pi_E_charge_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        pi_E_charge_U(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z) +
        X_R_stor_cumu_L(y, s, z) * pi_E_charge[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        X_R_stor_cumu_L(y, s, z) * kappa_E[y, p_star, s, z, d, t] +
        kappa_E_L(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        kappa_E_L(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_kappa_E_x_stor_R_p_star[y, s, z, d, t] >=
        -kappa_E_U(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        kappa_E_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] +
        kappa_E[y, p_star, s, z, d, t] * X_R_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_kappa_E_x_stor_R_p_star[y, s, z, d, t] <=
        X_R_stor_cumu_U(y, s, z) * kappa_E[y, p_star, s, z, d, t] -
        kappa_E_L(y, s, z, d, t) * X_R_stor_cumu_U(y, s, z) +
        kappa_E_L(y, s, z, d, t) * x_stor_R_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_kappa_E_x_stor_R_p_star[y, s, z, d, t] <=
        kappa_E_U(y, s, z, d, t) * x_stor_R_mce[y, s, z] -
        kappa_E_U(y, s, z, d, t) * X_R_stor_cumu_L(y, s, z) +
        X_R_stor_cumu_L(y, s, z) * kappa_E[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        mce_theta_C_energy_x_stor_C_p_star[y, s, z, d, t] >=
        X_C_stor_cumu_L(y, s, z) * theta_C_energy[y, p_star, s, z, d, t] +
        theta_C_energy_L(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        theta_C_energy_L(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_energy_x_stor_C_p_star[y, s, z, d, t] >=
        -theta_C_energy_U(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        theta_C_energy_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] +
        theta_C_energy[y, p_star, s, z, d, t] * X_C_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_energy_x_stor_C_p_star[y, s, z, d, t] <=
        X_C_stor_cumu_U(y, s, z) * theta_C_energy[y, p_star, s, z, d, t] -
        theta_C_energy_L(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        theta_C_energy_L(y, s, z, d, t) * x_stor_C_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_energy_x_stor_C_p_star[y, s, z, d, t] <=
        theta_C_energy_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        theta_C_energy_U(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z) +
        X_C_stor_cumu_L(y, s, z) * theta_C_energy[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        mce_theta_C_discharge_x_stor_C_p_star[y, s, z, d, t] >=
        X_C_stor_cumu_L(y, s, z) * theta_C_discharge[y, p_star, s, z, d, t] +
        theta_C_discharge_L(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        theta_C_discharge_L(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_discharge_x_stor_C_p_star[y, s, z, d, t] >=
        -theta_C_discharge_U(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        theta_C_discharge_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] +
        theta_C_discharge[y, p_star, s, z, d, t] * X_C_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_discharge_x_stor_C_p_star[y, s, z, d, t] <=
        X_C_stor_cumu_U(y, s, z) * theta_C_discharge[y, p_star, s, z, d, t] -
        theta_C_discharge_L(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        theta_C_discharge_L(y, s, z, d, t) * x_stor_C_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_discharge_x_stor_C_p_star[y, s, z, d, t] <=
        theta_C_discharge_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        theta_C_discharge_U(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z) +
        X_C_stor_cumu_L(y, s, z) * theta_C_discharge[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        X_C_stor_cumu_L(y, s, z) * theta_C_charge[y, p_star, s, z, d, t] +
        theta_C_charge_L(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        theta_C_charge_L(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_charge_x_stor_C_p_star[y, s, z, d, t] >=
        -theta_C_charge_U(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        theta_C_charge_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] +
        theta_C_charge[y, p_star, s, z, d, t] * X_C_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_charge_x_stor_C_p_star[y, s, z, d, t] <=
        X_C_stor_cumu_U(y, s, z) * theta_C_charge[y, p_star, s, z, d, t] -
        theta_C_charge_L(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        theta_C_charge_L(y, s, z, d, t) * x_stor_C_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_theta_C_charge_x_stor_C_p_star[y, s, z, d, t] <=
        theta_C_charge_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        theta_C_charge_U(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z) +
        X_C_stor_cumu_L(y, s, z) * theta_C_charge[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        mce_pi_C_charge_x_stor_C_p_star[y, s, z, d, t] >=
        X_C_stor_cumu_L(y, s, z) * pi_C_charge[y, p_star, s, z, d, t] +
        pi_C_charge_L(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        pi_C_charge_L(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_pi_C_charge_x_stor_C_p_star[y, s, z, d, t] >=
        -pi_C_charge_U(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        pi_C_charge_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] +
        pi_C_charge[y, p_star, s, z, d, t] * X_C_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_pi_C_charge_x_stor_C_p_star[y, s, z, d, t] <=
        X_C_stor_cumu_U(y, s, z) * pi_C_charge[y, p_star, s, z, d, t] -
        pi_C_charge_L(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        pi_C_charge_L(y, s, z, d, t) * x_stor_C_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_pi_C_charge_x_stor_C_p_star[y, s, z, d, t] <=
        pi_C_charge_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        pi_C_charge_U(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z) +
        X_C_stor_cumu_L(y, s, z) * pi_C_charge[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        ...
        X_C_stor_cumu_L(y, s, z) * kappa_C[y, p_star, s, z, d, t] +
        kappa_C_L(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        kappa_C_L(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_kappa_C_x_stor_C_p_star[y, s, z, d, t] >=
        -kappa_C_U(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        kappa_C_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] +
        kappa_C[y, p_star, s, z, d, t] * X_C_stor_cumu_U(y, s, z)
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_kappa_C_x_stor_C_p_star[y, s, z, d, t] <=
        X_C_stor_cumu_U(y, s, z) * kappa_C[y, p_star, s, z, d, t] -
        kappa_C_L(y, s, z, d, t) * X_C_stor_cumu_U(y, s, z) +
        kappa_C_L(y, s, z, d, t) * x_stor_C_mce[y, s, z]
    )
    @constraint(
        WMDER_IPP,
        ...
        mce_kappa_C_x_stor_C_p_star[y, s, z, d, t] <=
        kappa_C_U(y, s, z, d, t) * x_stor_C_mce[y, s, z] -
        kappa_C_U(y, s, z, d, t) * X_C_stor_cumu_L(y, s, z) +
        X_C_stor_cumu_L(y, s, z) * kappa_C[y, p_star, s, z, d, t]
    )
    ...
    @constraint(
        WMDER_IPP,
        Eq_cap_flow_lower[y in model_data.index_y, l in ipp.index_l],
        flow_cap[y, l] - ipp.trans_capacity(l, :min) >= 0
    )

    @constraint(
        WMDER_IPP,
        Eq_cap_flow_upper[y in model_data.index_y, l in ipp.index_l],
    ...
    @constraint(
        ...
        sum(
            model_data.omega(d) *
            delta_t *
            (y_E[y, p, rps, z, d, t] + y_C[y, p, rps, z, d, t]) for p in ipp.index_p,
            rps in ipp.index_rps, z in model_data.index_z, d in model_data.index_d,
            t in model_data.index_t
        ) -
        ipp.RPS(y) * sum(
            model_data.omega(d) * delta_t * ipp.Net_Load_my(y, z, d, t) for
    ...
    for y in model_data.index_y,
        ...
        ipp.LMP_my(y, z, d, t, :) .=
            ipp.miu_my(y, z, d, t) / (model_data.omega(d) * delta_t)
    end
    ...
    for y in model_data.index_y
        UCAP_p_star(y, :) .=
            sum(
                ipp.capacity_credit_E_my(y, z, k) * (
                    ipp.x_E_my(p_star, z, k) - sum(
                        ipp.x_R_my(Symbol(Int(y_symbol)), p_star, k, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) - ipp.x_R_cumu(p_star, k, z)
                ) for k in ipp.index_k_existing, z in model_data.index_z
            ) +
            sum(
                ipp.capacity_credit_C_my(y, z, k) * (
                    sum(
                        ipp.x_C_my(Symbol(Int(y_symbol)), p_star, k, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) + ipp.x_C_cumu(p_star, k, z)
                ) for k in ipp.index_k_new, z in model_data.index_z
            ) +
            sum(
                ipp.capacity_credit_stor_E_my(y, z, s) * (
                    ipp.x_stor_E_my(p_star, z, s) - sum(
                        ipp.x_stor_R_my(Symbol(Int(y_symbol)), p_star, s, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) - ipp.x_stor_R_cumu(p_star, s, z)
                ) for s in ipp.index_stor_existing, z in model_data.index_z
            ) +
            sum(
                ipp.capacity_credit_stor_C_my(y, z, s) * (
                    sum(
                        ipp.x_stor_C_my(Symbol(Int(y_symbol)), p_star, s, z) for y_symbol =
                            model_data.year(first(model_data.index_y)):model_data.year(y)
                    ) + ipp.x_stor_C_cumu(p_star, s, z)
                ) for s in ipp.index_stor_new, z in model_data.index_z
            )
            ...
            lower_level_dual_obj[1, y] =
                sum(
                    ...
                    sum(
                        ...
                        ipp.rho_E_my(p_star, k, z, d, t) *
                        value.(eta[y, p_star, k, z, d, t]) *
                        value.(x_R_mce[y, k, z]) for k in ipp.index_k_existing,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                ) - (
                    sum(
                        ...
                        ipp.rho_C_my(p_star, k, z, d, t) *
                        value.(lambda[y, p_star, k, z, d, t]) *
                        value.(x_C_mce[y, k, z]) for k in ipp.index_k_new,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                ) + (
                    sum(
                        value.(iota_min[y, l, d, t]) * ipp.trans_capacity(l, :min) for
                        l in ipp.index_l, d in model_data.index_d, t in model_data.index_t
                    ) - sum(
                        value.(iota_max[y, l, d, t]) * ipp.trans_capacity(l, :max) for
                        l in ipp.index_l, d in model_data.index_d, t in model_data.index_t
                    )
                ) - (
                    sum(
                        ...
                        ipp.stor_duration_existing(s) *
                        (ipp.x_stor_E_my(p, z, s) - ipp.x_stor_R_cumu(p, s, z)) for
                        p in ipp.index_p, s in ipp.index_stor_existing,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    ) - sum(
                        ipp.stor_duration_existing(s) *
                        value.(theta_E_energy[y, p_star, s, z, d, t]) *
                        value.(x_stor_R_mce[y, s, z]) for s in ipp.index_stor_existing,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                ) - (
                    sum(
                        ...
                        ipp.rte_stor_E_my(y, p_star, z, s) *
                        value.(theta_E_discharge[y, p_star, s, z, d, t]) *
                        value.(x_stor_R_mce[y, s, z]) for s in ipp.index_stor_existing,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                    ...
                    sum(
                        ...
                        value.(theta_E_charge[y, p_star, s, z, d, t]) *
                        value.(x_stor_R_mce[y, s, z]) for s in ipp.index_stor_existing,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                    ...
                    sum(
                        ...
                        ipp.stor_duration_existing(s) *
                        value.(pi_E_charge[y, p_star, s, z, d, t]) *
                        value.(x_stor_R_mce[y, s, z]) for s in ipp.index_stor_existing,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                    ...
                    sum(
                        ...
                        value.(kappa_E[y, p_star, s, z, d, t]) *
                        value.(x_stor_R_mce[y, s, z]) for s in ipp.index_stor_existing,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                    ...
                    sum(
                        ...
                        ipp.stor_duration_new(s) *
                        value.(theta_C_energy[y, p_star, s, z, d, t]) *
                        value.(x_stor_C_mce[y, s, z]) for s in ipp.index_stor_new,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                ) - (
                    sum(
                        ...
                        ipp.rte_stor_C_my(y, p_star, z, s) *
                        value.(theta_C_discharge[y, p_star, s, z, d, t]) *
                        value.(x_stor_C_mce[y, s, z]) for s in ipp.index_stor_new,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                    ...
                    sum(
                        value.(theta_C_charge[y, p, s, z, d, t]) *
                        ipp.x_stor_C_cumu(p, s, z) for p in ipp.index_p,
                        s in ipp.index_stor_new, z in model_data.index_z,
                        d in model_data.index_d, t in model_data.index_t
                    ) + sum(
                        value.(theta_C_charge[y, p_star, s, z, d, t]) *
                        value.(x_stor_C_mce[y, s, z]) for s in ipp.index_stor_new,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                    ...
                    sum(
                        ...
                        ipp.stor_duration_new(s) *
                        value.(pi_C_charge[y, p_star, s, z, d, t]) *
                        value.(x_stor_C_mce[y, s, z]) for s in ipp.index_stor_new,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                    ...
                    sum(
                        value.(kappa_C[y, p, s, z, d, t]) * ipp.x_stor_C_cumu(p, s, z) for
                        p in ipp.index_p, s in ipp.index_stor_new, z in model_data.index_z,
                        d in model_data.index_d, t in model_data.index_t
                    ) + sum(
                        value.(kappa_C[y, p_star, s, z, d, t]) *
                        value.(x_stor_C_mce[y, s, z]) for s in ipp.index_stor_new,
                        z in model_data.index_z, d in model_data.index_d,
                        t in model_data.index_t
                    )
                ) + sum(
                    value.(psi_E[y, p, s, z, d, model_data.index_t.elements[1]]) *
                    ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d
                ) - sum(
                    value.(pi_E_discharge[y, p, s, z, d, model_data.index_t.elements[1]]) *
                    ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d
                ) +
                sum(
                    value.(pi_E_charge[y, p, s, z, d, model_data.index_t.elements[1]]) *
                    ipp.initial_energy_existing_my(y, p, s, z, d) for p in ipp.index_p,
                    s in ipp.index_stor_existing, z in model_data.index_z,
                    d in model_data.index_d
                ) +
                sum(
                    value.(psi_C[y, p, s, z, d, model_data.index_t.elements[1]]) *
                    ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d
                ) - sum(
                    value.(pi_C_discharge[y, p, s, z, d, model_data.index_t.elements[1]]) *
                    ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d
                ) + sum(
                    value.(pi_C_charge[y, p, s, z, d, model_data.index_t.elements[1]]) *
                    ipp.initial_energy_new_my(y, p, s, z, d) for p in ipp.index_p,
                    s in ipp.index_stor_new, z in model_data.index_z,
                    d in model_data.index_d
                )
    ...
    lower_level_duality_gap =
        (lower_level_primal_obj .- lower_level_dual_obj) ./ abs.(lower_level_primal_obj)
    @info "lower level primal obj is $(lower_level_primal_obj)"
    @info "lower level dual obj is $(lower_level_dual_obj)"
    @info "lower level duality gap is $(lower_level_duality_gap)"
...
function welfare_calculation!(
    ...
    for y in model_data.index_y_fix, p in ipp.index_p
        fixed_om(y, p, :) .=
            sum(
                ipp.fom_E_my(y, p, k) * (
                    ipp.x_E_my(p, k) - sum(
                        ipp.x_R_my(Symbol(Int(y_symbol)), p, k) for y_symbol =
                            model_data.year(first(model_data.index_y_fix)):model_data.year(
                                y,
                            )
                    )
                ...
                y_symbol = model_data.year(first(model_data.index_y_fix)):model_data.year(y)
            )
    end
    ...
    RateBaseNoWC_new =
        make_keyed_array(model_data.index_y_fix, ipp.index_p, ipp.index_k_new)
    for y in model_data.index_y_fix, p in ipp.index_p, k in ipp.index_k_new
...
function update_cumulative!(model_data::HEMData, ipp::IPPGroup)
    for p in ipp.index_p, k in ipp.index_k_existing, z in model_data.index_z
        ipp.x_R_cumu(p, k, z, :) .= min(
            ipp.x_R_cumu(p, k, z) + ipp.x_R_my(first(model_data.index_y), p, k, z),
            ipp.x_E_my(p, z, k),
        )
    end
    ...
    for p in ipp.index_p, k in ipp.index_k_new, z in model_data.index_z
        ipp.x_C_cumu(p, k, z, :) .=
            ipp.x_C_cumu(p, k, z) + ipp.x_C_my(first(model_data.index_y), p, k, z)
    end

    for p in ipp.index_p, s in ipp.index_stor_existing, z in model_data.index_z
        ipp.x_stor_R_cumu(p, s, z, :) .= min(
            ipp.x_stor_R_cumu(p, s, z) +
            ipp.x_stor_R_my(first(model_data.index_y), p, s, z),
            ipp.x_stor_E_my(p, z, s),
        )
    end

    for p in ipp.index_p, s in ipp.index_stor_new, z in model_data.index_z
        ipp.x_stor_C_cumu(p, s, z, :) .=
            ipp.x_stor_C_cumu(p, s, z) + ipp.x_stor_C_my(first(model_data.index_y), p, s, z)
    end
end
