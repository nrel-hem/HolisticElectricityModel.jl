...
mutable struct IPPGroup <: AbstractIPPGroup
    ...
    # store current JuMP model for infeasible check
    current_jump_model::Any
end
...
function IPPGroup(input_filename::String, model_data::HEMData, id = DEFAULT_ID)
    ...
    FixedCostNew = make_axis_array(index_p, index_k_new)
    ...
    pvf_onm = make_axis_array(model_data.index_y, index_p)
    for y in model_data.index_y, p in index_p
...
function Lagrange_Sub_Investment_Retirement_Cap(
    ...
    ipp.current_jump_model = WMDER_IPP
...
function Lagrange_Sub_Dispatch_Cap(
    ...
    # set_optimizer_attribute(WMDER_IPP, "MIPFocus", 3)
...
function Lagrange_Feasible_Cap(
    ...
    WMDER_IPP = get_new_jump_model(hem_opts.MIP_solver)
    # set_optimizer_attribute(WMDER_IPP, "Presolve", 0)
...
function solve_agent_problem_ipp_cap(
    ...
    while optimality_gap >= tol && lagrange_iter <= max_iter_ipp
        @info "Solving investment retirement subproblem"
        ...
        @info "Solving dispatch subproblem"
        ...
        while ipp.obj_st1 + ipp.obj_st2 < ipp.obj_feasible
            @info "upper bound less than lower bound, change variable L"
            for y in model_data.index_y, k in ipp.index_k_existing
...
#     # set_optimizer_attribute(WMDER_IPP, "Presolve", 0)
#     # set_optimizer_attribute(WMDER_IPP, "MIPFocus", 3)
#     # set_optimizer_attribute(WMDER_IPP, "Cuts", 3)
...
#     # scaling_quadratic_var = 100
#     # scaling_objective = 1000000
#     # scaling_rps = 1000
#     # scaling_equations = 100

#     scaling_factor = 1
#     scaling_quadratic_var = 1
#     scaling_objective = 1
#     scaling_rps = 1
#     scaling_equations = 1
...
#                 ipp.capacity_credit_E_my[y, k] * (
#                     ipp.x_E_my[p_star, k] - sum(
#                         x_R[Symbol(Int(y_symbol)), k] * scaling_factor for y_symbol in
#                         model_data.year[first(model_data.index_y)]:model_data.year[y]
#                     ) - ipp.x_R_cumu[p_star, k]
#                 ) for k in ipp.index_k_existing
#             ) + sum(
#                 ipp.capacity_credit_C_my[y, k] * (
#                     sum(
#                         x_C[Symbol(Int(y_symbol)), k] * scaling_factor for y_symbol in
#                         model_data.year[first(model_data.index_y)]:model_data.year[y]
#                     ) + ipp.x_C_cumu[p_star, k]
#                 ) for k in ipp.index_k_new
#             )
...
#         UCAP_total_var =
#             y -> begin
#                 UCAP_p_star[y] * scaling_quadratic_var +
#                 sum(
#                     ipp.capacity_credit_E_my[y, k] * (
#                         ipp.x_E_my[p, k] - sum(
#                             ipp.x_R_my[Symbol(Int(y_symbol)), p, k] for y_symbol in
...
#             ipp.pvf_onm[y, p_star] * (
#                 UCAP_p_star[y] * scaling_quadratic_var * (
#                     ipp.Capacity_intercept_my[y] + ipp.Capacity_slope_my[y] * UCAP_total_var(y)
#                 )
#             ) -
...
#             #   capex * cap new for gen type
#             ipp.pvf_cap[y, p_star] *
#             sum(ipp.CapEx_my[y, p_star, k] * x_C[y, k] * scaling_factor for k in ipp.index_k_new) +

#             # Linearized profit term
#             ipp.pvf_onm[y, p_star] * (
#                 sum(
#                     miu[y, t] * scaling_factor * (
#                         sum(
#                             customers.gamma[h] * customers.d_my[y, h, t] for
#                             h in model_data.index_h
#                         ) + ipp.eximport_my[y, t] - sum(
...
#                         p in ipp.index_p
#                     ) - sum(
#                         eta[y, p_star, k, t] * scaling_factor *
#                         ipp.rho_E_my[p_star, k, t] *
#                         (
#                             ipp.x_E_my[p_star, k] - sum(
#                                 ipp.x_R_my[Symbol(Int(y_symbol)), p_star, k] for
...
#                         p in ipp.index_p
#                     ) - sum(
#                         lambda[y, p_star, k, t] * scaling_factor *
#                         ipp.rho_C_my[p_star, k, t] *
#                         (
#                             sum(
#                                 ipp.x_C_my[Symbol(Int(y_symbol)), p_star, k] for
...
#                 ) - (
#                     sum(
#                         model_data.omega[t] * (ipp.v_E_my[y, p, k, t] * y_E[y, p, k, t] * scaling_factor) for
#                         t in model_data.index_t, k in ipp.index_k_existing,
#                         p in ipp.index_p
#                     ) - sum(
#                         model_data.omega[t] *
#                         (ipp.v_E_my[y, p_star, k, t] * y_E[y, p_star, k, t] * scaling_factor) for
#                         t in model_data.index_t, k in ipp.index_k_existing
#                     )
#                 ) - (
#                     sum(
#                         model_data.omega[t] * (ipp.v_C_my[y, p, k, t] * y_C[y, p, k, t] * scaling_factor) for
#                         t in model_data.index_t, k in ipp.index_k_new, p in ipp.index_p
#                     ) - sum(
#                         model_data.omega[t] *
#                         (ipp.v_C_my[y, p_star, k, t] * y_C[y, p_star, k, t] * scaling_factor) for
#                         t in model_data.index_t, k in ipp.index_k_new
#                     )
#                 ) -
...
#     @objective(WMDER_IPP, Max, objective_function / scaling_objective)

#     @constraint(
#         WMDER_IPP,
#         Eq_UCAP_p_star[y in model_data.index_y],
#         UCAP_p_star[y] * scaling_quadratic_var / scaling_equations == UCAP_p_star_var(y) / scaling_equations
#     )
...
#         Eq_sigma[y in model_data.index_y, k in ipp.index_k_existing],
#         (ipp.x_E_my[p_star, k] - sum(
#             x_R[Symbol(Int(y_symbol)), k] * scaling_factor for
#             y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
#         ) - ipp.x_R_cumu[p_star, k])  / scaling_equations >= 0
#     )
...
#             t in model_data.index_t,
#         ],
#         y_E[y, p, k, t] <= ipp.B1GM.value * u_y_E[y, p, k, t] / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_E_p_2[
...
#             t in model_data.index_t,
#         ],
#         model_data.omega[t] * ipp.v_E_my[y, p, k, t] / scaling_factor - miu[y, t] + eta[y, p, k, t] <=
#         ipp.B1GM.value * (1 - u_y_E[y, p, k, t]) / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_E_p_3[
...
#             t in model_data.index_t,
#         ],
#         model_data.omega[t] * ipp.v_E_my[y, p, k, t] / scaling_factor - miu[y, t] + eta[y, p, k, t] >= 0
#     )
...
#             t in model_data.index_t,
#         ],
#         y_C[y, p, k, t] <= ipp.B1GM.value * u_y_C[y, p, k, t] / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_C_p_2[
...
#             t in model_data.index_t,
#         ],
#         model_data.omega[t] * ipp.v_C_my[y, p, k, t] / scaling_factor - miu[y, t] + lambda[y, p, k, t] <=
#         ipp.B1GM.value * (1 - u_y_C[y, p, k, t]) / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_C_p_3[
...
#             t in model_data.index_t,
#         ],
#         model_data.omega[t] * ipp.v_C_my[y, p, k, t] / scaling_factor - miu[y, t] + lambda[y, p, k, t] >= 0
#     )
...
#         WMDER_IPP,
#         Eq_miu_1[y in model_data.index_y, t in model_data.index_t],
#         miu[y, t] <= ipp.B1GM.value * u_miu[y, t] / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_miu_2[y in model_data.index_y, t in model_data.index_t],
#         supply_demand_balance(y, t) <= ipp.B1GM.value * (1 - u_miu[y, t]) / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_miu_3[y in model_data.index_y, t in model_data.index_t],
...
#             t in model_data.index_t,
#         ],
#         eta[y, p_star, k, t] <= ipp.B1GM.value * u_eta[y, p_star, k, t] / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_eta_p_star_2[
...
#         ],
#         ipp.rho_E_my[p_star, k, t] * (
#             ipp.x_E_my[p_star, k] / scaling_factor - sum(
#                 x_R[Symbol(Int(y_symbol)), k] for
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
#             ) - ipp.x_R_cumu[p_star, k]
#         ) - y_E[y, p_star, k, t] <= ipp.B1GM.value * (1 - u_eta[y, p_star, k, t]) / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_eta_p_star_3[
...
#                 t in model_data.index_t,
#             ],
#             eta[y, p, k, t] <= ipp.B1GM.value * u_eta[y, p, k, t] / scaling_factor
#         )
#         @constraint(
#             WMDER_IPP,
#             Eq_eta_p_other_2[
...
#             ],
#             ipp.rho_E_my[p, k, t] * (
#                 ipp.x_E_my[p, k] / scaling_factor - sum(
#                     ipp.x_R_my[Symbol(Int(y_symbol)), p, k] for y_symbol in
#                     model_data.year[first(model_data.index_y)]:model_data.year[y]
#                 ) - ipp.x_R_cumu[p, k]
#             ) - y_E[y, p, k, t] <= ipp.B1GM.value * (1 - u_eta[y, p, k, t]) / scaling_factor
#         )
#         @constraint(
#             WMDER_IPP,
#             Eq_eta_p_other_3[
...
#             t in model_data.index_t,
#         ],
#         lambda[y, p_star, k, t] <= ipp.B1GM.value * u_lambda[y, p_star, k, t] / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_lambda_p_star_2[
...
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
#             ) + ipp.x_C_cumu[p_star, k]
#         ) - y_C[y, p_star, k, t] <= ipp.B1GM.value * (1 - u_lambda[y, p_star, k, t]) / scaling_factor
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_lambda_p_star_3[
...
#                 t in model_data.index_t,
#             ],
#             lambda[y, p, k, t] <= ipp.B1GM.value * u_lambda[y, p, k, t] / scaling_factor
#         )
#         @constraint(
#             WMDER_IPP,
#             Eq_lambda_p_other_2[
...
#         WMDER_IPP,
#         Eq_xi[y in model_data.index_y, t in model_data.index_t],
#         planning_reserves(y, t) / scaling_equations >= 0
#     )
...
#         WMDER_IPP,
#         Eq_xi_cap[y in model_data.index_y],
#         planning_reserves_cap(y) / scaling_equations >= 0
#     )
...
#         Eq_rps[y in model_data.index_y],
#         sum(
#             model_data.omega[t] * (y_E[y, p, rps, t] + y_C[y, p, rps, t]) * scaling_factor for
#             rps in ipp.index_rps, t in model_data.index_t, p in ipp.index_p
#         ) / scaling_rps / scaling_equations -
#         ipp.RPS[y] *
#         sum(model_data.omega[t] * ipp.Net_Load_my[y, t] for t in model_data.index_t) / scaling_rps / scaling_equations >= 0
#     )
...
########### MPPDC -- McCormick-envelope Relaxation ###########
function solve_agent_problem_ipp_cap(
    ipp::IPPGroup,
    ...
    WMDER_IPP = get_new_jump_model(hem_opts.MIP_solver)
    MPPDCMER_lower = get_new_jump_model(hem_opts.MIP_solver)
    # set_optimizer_attribute(WMDER_IPP, "Presolve", 0)
    # set_optimizer_attribute(WMDER_IPP, "PreQLinearize", 2)
    # set_optimizer_attribute(WMDER_IPP, "MIPFocus", 3)
    ...
    # use lower level optimization results to set variable bounds for McCormick-envelope Relaxation
    @variable(
        MPPDCMER_lower,
    ...
    # setvalue(x_C[Symbol("2019"), :GasCC], 1162.40)
    # setvalue(x_C[Symbol("2019"), Symbol("Gas-CT")], 691.60)
    # setvalue(x_C[Symbol("2019"), Symbol("UPV")], 804.72)
    # setvalue(x_R[Symbol("2019"), Symbol("Gas-CC")], 1162.40)
    # setvalue(x_R[Symbol("2019"), Symbol("Gas-CT")], 0.0)
    # setvalue(x_R[Symbol("2019"), :UPV], 0.0)
    ...
    @constraint(
        WMDER_IPP,
        Eq_UCAP_p_star[y in model_data.index_y],
        UCAP_p_star[y] == UCAP_p_star_var(y)
    )
    ...
    TimerOutputs.@timeit HEM_TIMER "optimize! MPPDC McCormic Envelope Relaxation" begin
        optimize!(WMDER_IPP)
    end
...
function solve_agent_problem_ipp_cap(
    ...
    # get_optimizer_attribute(WMDER_IPP, "MIPTOL")
    # set_optimizer_attributes(WMDER_IPP, "tol" => 1e-4, "max_iter" => 500)
    ...
    for y in model_data.index_y,
        ...
        ipp.y_E_my[y, p, k, t] = value.(y_E[y, p, k, t])
        ipp.eta_my[y, p, k, t] = value.(eta[y, p, k, t])
    end
    for y in model_data.index_y,
        p in ipp.index_p,
        k in ipp.index_k_new,
        ...
        ipp.y_C_my[y, p, k, t] = value.(y_C[y, p, k, t])
        ipp.lambda_my[y, p, k, t] = value.(lambda[y, p, k, t])
    end
    for y in model_data.index_y, t in model_data.index_t
        ipp.miu_my[y, t] = value.(miu[y, t])
    end
...
#     # set up customized big M parameters
#     B1GM_customized = ParamAxisArray(ipp.rho_E_my, "B1GM_customized")
#     fill!(B1GM_customized, NaN)
#     for p in ipp.index_p, k in ipp.index_k_existing, t in model_data.index_t
#         B1GM_customized[p, k, t] = ipp.rho_E_my[p, k, t] * ipp.x_E_my[p, k]
#     end
#     @constraint(
#         WMDER_IPP,
#         Eq_y_E_p_1[
...
#             t in model_data.index_t,
#         ],
#         y_E[y, p, k, t] <= B1GM_customized[p, k, t] * u_y_E[y, p, k, t]
#     )
#     B1GM_customized = ParamAxisArray(ipp.v_E_my, "B1GM_customized")
#     fill!(B1GM_customized, NaN)
#     for y in model_data.index_y, p in ipp.index_p, k in ipp.index_k_existing, t in model_data.index_t
#         B1GM_customized[y, p, k, t] = model_data.omega[t] * ipp.v_E_my[y, p, k, t]
#     end
#     @constraint(
#         WMDER_IPP,
#         Eq_y_E_p_2[
...
#         ],
#         model_data.omega[t] * ipp.v_E_my[y, p, k, t] - miu[y, t] + eta[y, p, k, t] <=
#         B1GM_customized[y, p, k, t] * (1 - u_y_E[y, p, k, t])
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_E_p_3[
...
#     B1GM_customized = ParamAxisArray(ipp.rho_C_my, "B1GM_customized")
#     fill!(B1GM_customized, NaN)
#     for p in ipp.index_p, k in ipp.index_k_new, t in model_data.index_t
#         B1GM_customized[p, k, t] = ipp.rho_C_my[p, k, t] * 5000
#     end
#     @constraint(
#         WMDER_IPP,
#         Eq_y_C_p_1[
...
#             t in model_data.index_t,
#         ],
#         y_C[y, p, k, t] <= B1GM_customized[p, k, t] * u_y_C[y, p, k, t]
#     )
#     B1GM_customized = ParamAxisArray(ipp.v_C_my, "B1GM_customized")
#     fill!(B1GM_customized, NaN)
#     for y in model_data.index_y, p in ipp.index_p, k in ipp.index_k_new, t in model_data.index_t
#         B1GM_customized[y, p, k, t] = model_data.omega[t] * ipp.v_C_my[y, p, k, t]
#     end
#     @constraint(
#         WMDER_IPP,
#         Eq_y_C_p_2[
...
#         ],
#         model_data.omega[t] * ipp.v_C_my[y, p, k, t] - miu[y, t] + lambda[y, p, k, t] <=
#         B1GM_customized[y, p, k, t] * (1 - u_y_C[y, p, k, t])
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_C_p_3[
...
#     B1GM_customized = ParamAxisArray(ipp.miu_my, "B1GM_customized")
#     fill!(B1GM_customized, NaN)
#     for y in model_data.index_y, t in model_data.index_t
#         B1GM_customized[y, t] = max(maximum(model_data.omega[t] * ipp.v_C_my[y, p, k, t] 
#         for p in ipp.index_p, k in ipp.index_k_new), 
#         maximum(model_data.omega[t] * ipp.v_E_my[y, p, k, t] 
#         for p in ipp.index_p, k in ipp.index_k_existing))
#     end
#     @constraint(
#         WMDER_IPP,
#         Eq_miu_1[y in model_data.index_y, t in model_data.index_t],
#         miu[y, t] <= B1GM_customized[y, t] * u_miu[y, t]
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_miu_2[y in model_data.index_y, t in model_data.index_t],
#         supply_demand_balance(y, t) <= B1GM_customized[y, t] * (1 - u_miu[y, t])
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_miu_3[y in model_data.index_y, t in model_data.index_t],
...
#             t in model_data.index_t,
#         ],
#         eta[y, p_star, k, t] <= B1GM_customized[y, p_star, k, t] * u_eta[y, p_star, k, t]
#     )
#     B1GM_customized = ParamAxisArray(ipp.rho_E_my, "B1GM_customized")
#     fill!(B1GM_customized, NaN)
#     for p in ipp.index_p, k in ipp.index_k_existing, t in model_data.index_t
...
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
#             ) - ipp.x_R_cumu[p_star, k]
#         ) - y_E[y, p_star, k, t] <= B1GM_customized[p_star, k, t] * (1 - u_eta[y, p_star, k, t])
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_eta_p_star_3[
...
#                 t in model_data.index_t,
#             ],
#             eta[y, p, k, t] <= B1GM_customized[y, p, k, t] * u_eta[y, p, k, t]
#         )
#         B1GM_customized = ParamAxisArray(ipp.rho_E_my, "B1GM_customized")
#         fill!(B1GM_customized, NaN)
#         for p in ipp.index_p, k in ipp.index_k_existing, t in model_data.index_t
...
#                     model_data.year[first(model_data.index_y)]:model_data.year[y]
#                 ) - ipp.x_R_cumu[p, k]
#             ) - y_E[y, p, k, t] <= B1GM_customized[p, k, t] * (1 - u_eta[y, p, k, t])
#         )
#         @constraint(
#             WMDER_IPP,
#             Eq_eta_p_other_3[
...
#             t in model_data.index_t,
#         ],
#         lambda[y, p_star, k, t] <= B1GM_customized[y, p_star, k, t] * u_lambda[y, p_star, k, t]
#     )
#     B1GM_customized = ParamAxisArray(ipp.rho_C_my, "B1GM_customized")
#     fill!(B1GM_customized, NaN)
#     for p in ipp.index_p, k in ipp.index_k_new, t in model_data.index_t
...
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
#             ) + ipp.x_C_cumu[p_star, k]
#         ) - y_C[y, p_star, k, t] <= B1GM_customized[p_star, k, t] * (1 - u_lambda[y, p_star, k, t])
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_lambda_p_star_3[
...
#                 t in model_data.index_t,
#             ],
#             lambda[y, p, k, t] <= B1GM_customized[y, p, k, t] * u_lambda[y, p, k, t]
#         )
#         B1GM_customized = ParamAxisArray(ipp.rho_C_my, "B1GM_customized")
#         fill!(B1GM_customized, NaN)
#         for p in ipp.index_p, k in ipp.index_k_new, t in model_data.index_t
...
#                     model_data.year[first(model_data.index_y)]:model_data.year[y]
#                 ) + ipp.x_C_cumu[p, k]
#             ) - y_C[y, p, k, t] <= B1GM_customized[p, k, t] * (1 - u_lambda[y, p, k, t])
#         )
#         @constraint(
#             WMDER_IPP,
#             Eq_lambda_p_other_3[
...
#     @variable(
#         WMDER_IPP,
#         v_y_E[model_data.index_y, ipp.index_p, ipp.index_k_existing, model_data.index_t, 1:2] >= 0,
#     )
#     for y in model_data.index_y, p in ipp.index_p, k in ipp.index_k_existing, t in model_data.index_t
#         @constraint(WMDER_IPP, v_y_E[y, p, k, t, :] in SOS1())
#     end
...
#     @variable(
#         WMDER_IPP,
#         v_y_C[model_data.index_y, ipp.index_p, ipp.index_k_new, model_data.index_t, 1:2] >= 0,
#     )
#     for y in model_data.index_y, p in ipp.index_p, k in ipp.index_k_new, t in model_data.index_t
#         @constraint(WMDER_IPP, v_y_C[y, p, k, t, :] in SOS1())
#     end

#     @variable(WMDER_IPP, u_miu[model_data.index_y, model_data.index_t])
#     @variable(
#         WMDER_IPP,
#         v_miu[model_data.index_y, model_data.index_t, 1:2],
#     )
#     for y in model_data.index_y, t in model_data.index_t
#         @constraint(WMDER_IPP, v_miu[y, t, :] in SOS1())
#     end
...
#     @variable(
#         WMDER_IPP,
#         v_eta[model_data.index_y, ipp.index_p, ipp.index_k_existing, model_data.index_t, 1:2] >= 0,
#     )
#     for y in model_data.index_y, p in ipp.index_p, k in ipp.index_k_existing, t in model_data.index_t
#         @constraint(WMDER_IPP, v_eta[y, p, k, t, :] in SOS1())
#     end
...
#     @variable(
#         WMDER_IPP,
#         v_lambda[model_data.index_y, ipp.index_p, ipp.index_k_new, model_data.index_t, 1:2] >= 0,
#     )
#     for y in model_data.index_y, p in ipp.index_p, k in ipp.index_k_new, t in model_data.index_t
#         @constraint(WMDER_IPP, v_lambda[y, p, k, t, :] in SOS1())
#     end
...
#             t in model_data.index_t,
#         ],
#         u_y_E[y, p, k, t] - (v_y_E[y, p, k, t, 1] + v_y_E[y, p, k, t, 2]) == 0
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_E_p_3[
...
#             t in model_data.index_t,
#         ],
#         u_y_E[y, p, k, t] == (y_E[y, p, k, t] + model_data.omega[t] * ipp.v_E_my[y, p, k, t] - miu[y, t] + eta[y, p, k, t]) / 2
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_E_p_4[
...
#             t in model_data.index_t,
#         ],
#         v_y_E[y, p, k, t, 1] - v_y_E[y, p, k, t, 2] == (y_E[y, p, k, t] - (model_data.omega[t] * ipp.v_E_my[y, p, k, t] - miu[y, t] + eta[y, p, k, t])) / 2
#     )
...
#             t in model_data.index_t,
#         ],
#         u_y_C[y, p, k, t] - (v_y_C[y, p, k, t, 1] + v_y_C[y, p, k, t, 2]) == 0
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_C_p_3[
...
#             t in model_data.index_t,
#         ],
#         u_y_C[y, p, k, t] == (y_C[y, p, k, t] + model_data.omega[t] * ipp.v_C_my[y, p, k, t] - miu[y, t] + lambda[y, p, k, t]) / 2
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_y_C_p_4[
...
#             t in model_data.index_t,
#         ],
#         v_y_C[y, p, k, t, 1] - v_y_C[y, p, k, t, 2] == (y_C[y, p, k, t] - (model_data.omega[t] * ipp.v_C_my[y, p, k, t] - miu[y, t] + lambda[y, p, k, t])) / 2
#     )
...
#         WMDER_IPP,
#         Eq_miu_2[y in model_data.index_y, t in model_data.index_t],
#         u_miu[y, t] - (v_miu[y, t, 1] + v_miu[y, t, 2]) == 0
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_miu_3[y in model_data.index_y, t in model_data.index_t],
#         u_miu[y, t] == (miu[y, t] + supply_demand_balance(y, t)) / 2
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_miu_4[y in model_data.index_y, t in model_data.index_t],
#         v_miu[y, t, 1] - v_miu[y, t, 2] == (miu[y, t] - supply_demand_balance(y, t)) / 2
#     )
...
#             t in model_data.index_t,
#         ],
#         u_eta[y, p_star, k, t] - (v_eta[y, p_star, k, t, 1] + v_eta[y, p_star, k, t, 2]) == 0
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_eta_p_star_3[
...
#         ],
#         u_eta[y, p_star, k, t] == 
#         (eta[y, p_star, k, t] + ipp.rho_E_my[p_star, k, t] * (
#             ipp.x_E_my[p_star, k] - sum(
#                 x_R[Symbol(Int(y_symbol)), k] for
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
#             ) - ipp.x_R_cumu[p_star, k]
...
#             t in model_data.index_t,
#         ],
#         v_eta[y, p_star, k, t, 1] - v_eta[y, p_star, k, t, 2] == 
#         (eta[y, p_star, k, t] - (ipp.rho_E_my[p_star, k, t] * (
#             ipp.x_E_my[p_star, k] - sum(
#                 x_R[Symbol(Int(y_symbol)), k] for
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
...
#                 t in model_data.index_t,
#             ],
#             u_eta[y, p, k, t] - (v_eta[y, p, k, t, 1] + v_eta[y, p, k, t, 2]) == 0
#         )
#         @constraint(
#             WMDER_IPP,
#             Eq_eta_p_other_3[
...
#             ],
#             u_eta[y, p, k, t] == 
#             (eta[y, p, k, t] + ipp.rho_E_my[p, k, t] * (
#                 ipp.x_E_my[p, k] - sum(
#                     ipp.x_R_my[Symbol(Int(y_symbol)), p, k] for y_symbol in
#                     model_data.year[first(model_data.index_y)]:model_data.year[y]
#                 ) - ipp.x_R_cumu[p, k]
...
#                 t in model_data.index_t,
#             ],
#             v_eta[y, p, k, t, 1] - v_eta[y, p, k, t, 2] == 
#             (eta[y, p, k, t] - (ipp.rho_E_my[p, k, t] * (
#                 ipp.x_E_my[p, k] - sum(
#                     ipp.x_R_my[Symbol(Int(y_symbol)), p, k] for y_symbol in
#                     model_data.year[first(model_data.index_y)]:model_data.year[y]
...
#             t in model_data.index_t,
#         ],
#         u_lambda[y, p_star, k, t] - (v_lambda[y, p_star, k, t, 1] + v_lambda[y, p_star, k, t, 2]) == 0
#     )
#     @constraint(
#         WMDER_IPP,
#         Eq_lambda_p_star_3[
...
#             t in model_data.index_t,
#         ],
#         u_lambda[y, p_star, k, t] == 
#         (lambda[y, p_star, k, t] + ipp.rho_C_my[p_star, k, t] * (
#             sum(
#                 x_C[Symbol(Int(y_symbol)), k] for
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
#             ) + ipp.x_C_cumu[p_star, k]
...
#             t in model_data.index_t,
#         ],
#         v_lambda[y, p_star, k, t, 1] - v_lambda[y, p_star, k, t, 2] == 
#         (lambda[y, p_star, k, t] - (ipp.rho_C_my[p_star, k, t] * (
#             sum(
#                 x_C[Symbol(Int(y_symbol)), k] for
#                 y_symbol in model_data.year[first(model_data.index_y)]:model_data.year[y]
...
#                 t in model_data.index_t,
#             ],
#             u_lambda[y, p, k, t] - (v_lambda[y, p, k, t, 1] + v_lambda[y, p, k, t, 2]) == 0
#         )
#         @constraint(
#             WMDER_IPP,
#             Eq_lambda_p_other_3[
...
#             ],
#             u_lambda[y, p, k, t] == 
#             (lambda[y, p, k, t] + ipp.rho_C_my[p, k, t] * (
#                 sum(
#                     ipp.x_C_my[Symbol(Int(y_symbol)), p, k] for y_symbol in
#                     model_data.year[first(model_data.index_y)]:model_data.year[y]
#                 ) + ipp.x_C_cumu[p, k]
...
#                 t in model_data.index_t,
#             ],
#             v_lambda[y, p, k, t, 1] - v_lambda[y, p, k, t, 2] == 
#             (lambda[y, p, k, t] - (ipp.rho_C_my[p, k, t] * (
#                 sum(
#                     ipp.x_C_my[Symbol(Int(y_symbol)), p, k] for y_symbol in
#                     model_data.year[first(model_data.index_y)]:model_data.year[y]
...
function welfare_calculation!(
    ...
    IPP_Revenue_p = make_axis_array(model_data.index_y_fix, ipp.index_p)
    ...
    operational_cost = make_axis_array(model_data.index_y_fix, ipp.index_p)
    for y in model_data.index_y_fix, p in ipp.index_p
        operational_cost[y, p] = energy_cost[y, p] + fixed_om[y, p]
    end
    working_capital = make_axis_array(model_data.index_y_fix, ipp.index_p)
    for y in model_data.index_y_fix, p in ipp.index_p
    ...
    ADITNew = make_axis_array(model_data.index_y_fix, ipp.index_p, ipp.index_k_new)
    ...
    RateBaseNoWC_new = make_axis_array(model_data.index_y_fix, ipp.index_p, ipp.index_k_new)
    for y in model_data.index_y_fix, p in ipp.index_p, k in ipp.index_k_new
    ...
    debt_interest = make_axis_array(model_data.index_y_fix, ipp.index_p)
    for y in model_data.index_y_fix, p in ipp.index_p
        debt_interest[y, p] = rate_base[y, p] * ipp.DebtRatio[p] * ipp.COD[p]
    end

    depreciation = make_axis_array(model_data.index_y_fix, ipp.index_p)
    for y in model_data.index_y_fix, p in ipp.index_p
        depreciation[y, p] =
            sum(
    ...
    depreciation_tax = make_axis_array(model_data.index_y_fix, ipp.index_p)
    for y in model_data.index_y_fix, p in ipp.index_p
    ...
    income_tax = make_axis_array(model_data.index_y_fix, ipp.index_p)
    for y in model_data.index_y_fix, p in ipp.index_p
        income_tax[y, p] =
            (
    ...
    IPP_Cost_p = make_axis_array(model_data.index_y_fix, ipp.index_p)
    for y in model_data.index_y_fix, p in ipp.index_p
        IPP_Cost_p[y, p] =
            debt_interest[y, p] +
            income_tax[y, p] +
