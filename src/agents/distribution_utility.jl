...
function solve_agent_problem!(
    ...
    norm_inputs = DataFrame(id = ["saidi", "dpv", "total_sales",
                                  "residential", "commercial",
                                  "industrial", "distribution_cost"],
                            min = [0, 0.02, 33321,
                                   3664, 820,
                                   0, 1851740],
                            max = [968.6, 3018.362, 131738016,
                                   4489462, 619752,
                                   33562,3236569353])
    function norm(data, norm_variable)
        data = (data - norm_inputs[norm_inputs.id .==norm_variable,"min"])/
                (norm_inputs[norm_inputs.id .==norm_variable,"max"] - 
                norm_inputs[norm_inputs.id .==norm_variable,"min"])
        return data
    end;
                                
    function reverse_norm(data, norm_variable)
        data = data * (norm_inputs[norm_inputs.id .==norm_variable,"max"] - 
                norm_inputs[norm_inputs.id .==norm_variable,"min"]) + 
                norm_inputs[norm_inputs.id .==norm_variable,"min"]
        return data
    end;
    ...
    dpv_pca = sum(customers.x_DG_E[h, m] for h in model_data.index_h, m in customers.index_m)

    regulator.distribution_cost[reg_year_index] = 
        reverse_norm(distribution_cost_model.constant +
        distribution_cost_model.saidi_coefficient * norm(distribution_utility.SAIDI[reg_year_index], "saidi") +
        distribution_cost_model.dpv_coefficient * norm(dpv_pca, "dpv")  +
        distribution_cost_model.total_sales_coefficient * norm(total_sale, "total_sale") +
        distribution_cost_model.residential_customer_coefficient * norm(customers.gamma[:Residential], "residential") +
        distribution_cost_model.commercial_customer_coefficient * norm(customers.gamma[:Commercial], "commercial") +
        distribution_cost_model.industrial_customer_coefficient * norm(customers.gamma[:Industrial], "industrial"),
        "distribution_cost")
